...
class Tambah_Data_Baru_Master_Controller extends CI_Controller {
            ...
            if (!empty($q['required'])) {
                if ($kategori === 'Pakan') {
                    ...
                    if ($field === 'nama_pakan' && $tipe_ternak_pakan !== 'Layer') {
                        ...
                        $should_be_required = true;
                    } elseif ($field === 'layer_pakai_pakan_cp' && $tipe_ternak_pakan === 'Layer' && $pilihan_pakan_layer === 'CP') {
                        $should_be_required = true;
                    } elseif ($field === 'layer_selain_pakan_cp' && $tipe_ternak_pakan === 'Layer' && $pilihan_pakan_layer === 'Non CP') {
                        $should_be_required = true;
                    } elseif (!in_array($field, ['nama_pakan', 'pilihan_pakan', 'layer_pakai_pakan_cp', 'layer_selain_pakan_cp'])) {
        ...
        if ($kategori === 'Pakan') {
            ...
            if (isset($save_data['tipe_ternak']) && $save_data['tipe_ternak'] === 'Layer') {
                if (isset($save_data['pilihan_pakan'])) {
                    if ($save_data['pilihan_pakan'] === 'CP' && !empty($save_data['layer_pakai_pakan_cp'])) {
                        // If 'CP' is chosen, use its value for nama_pakan
                        $save_data['nama_pakan'] = $save_data['layer_pakai_pakan_cp'];
                    } elseif ($save_data['pilihan_pakan'] === 'Non CP' && !empty($save_data['layer_selain_pakan_cp'])) {
                        // If 'Non CP' is chosen, use its value for nama_pakan
                        $save_data['nama_pakan'] = $save_data['layer_selain_pakan_cp'];
                    }
                }
            }

            // Always remove the temporary fields for the Pakan category before saving.
            // These fields do not exist in the database table.
            // unset($save_data['layer_pakai_pakan_cp']);
            // unset($save_data['layer_selain_pakan_cp']);
        }
        // === AKHIR PERUBAHAN ===

        // === PERUBAHAN UTAMA: LOGIKA PENYIMPANAN PAKAN LAYER ===
        // if ($kategori === 'Pakan' && isset($save_data['tipe_ternak']) && $save_data['tipe_ternak'] === 'Layer') {
        //     if (isset($save_data['pilihan_pakan'])) {
        //         if ($save_data['pilihan_pakan'] === 'CP' && !empty($save_data['layer_pakai_pakan_cp'])) {
        //             // Jika pilih CP, gunakan nilainya sebagai nama_pakan
        //             $save_data['nama_pakan'] = $save_data['layer_pakai_pakan_cp'];
        //         } elseif ($save_data['pilihan_pakan'] === 'Non CP' && !empty($save_data['layer_selain_pakan_cp'])) {
        //             // Jika pilih Non CP, gunakan nilainya sebagai nama_pakan
        //             $save_data['nama_pakan'] = $save_data['layer_selain_pakan_cp'];
        //         }
        //     }
        //     // Hapus field sementara agar tidak mencoba disimpan ke DB jika tidak ada kolomnya
        //     unset($save_data['layer_pakai_pakan_cp']);
        //     unset($save_data['layer_selain_pakan_cp']);
        // }
        // === AKHIR PERUBAHAN ===
    ...
    private function _save_data($kategori, $save_data, $user, $page) {
        ...
        switch($kategori) {
            ...
            case 'Sub Agen':
                ...
                $this->_add_to_options($page, $save_data['nama_subagen'], null, $user['master_area_id']);
                break;
    ...
    private function _add_to_options($page, $option_text, $master_sub_area_id, $master_area_id = null) {
        ...
        foreach ($questions as $question) {
            ...
            if (in_array($page, ['master_subagen', 'master_agen', 'master_kemitraan']) && !empty($master_area_id)) {
                $options_data['master_area_id'] = $master_area_id;
            } elseif ($page == 'master_farm' && !empty($master_sub_area_id) && !empty($master_area_id)) {
    ...
    if (isset($save_data['tipe_ternak']) && $save_data['tipe_ternak'] === 'Layer') {
        if (isset($save_data['pilihan_pakan'])) {
            if ($save_data['pilihan_pakan'] === 'CP') {
                $target_field_name = 'layer_pakai_pakan_cp';
            } elseif ($save_data['pilihan_pakan'] === 'Non CP') {
                $target_field_name = 'layer_selain_pakan_cp';
            }
        }
    }
