...
<head>
    ...
    <style>
        ...
        /* Hide jenis kasus section by default */
        .jenis-kasus-section {
            display: none !important;
            visibility: hidden !important;
            opacity: 0 !important;
        ...
        /* Force hide jenis kasus by default - more aggressive approach */
        .form-group:has(label:contains('Jenis Kasus')) {
            display: none !important;
            visibility: hidden !important;
            opacity: 0 !important;
            height: 0 !important;
    ...
    <script>
        // Immediate hiding of jenis kasus fields and sections
        (function() {
            function hideJenisKasusFieldsImmediately() {
                console.log('Executing hideJenisKasusFieldsImmediately...');
                
                // Check if Kasus is selected in tujuan kunjungan
                const tujuanKunjunganRadios = document.querySelectorAll('input[type="radio"]');
                let kasusSelected = false;
                ...
                tujuanKunjunganRadios.forEach(function(radio) {
                    ...
                    if (label && (label.textContent.includes('Tujuan Kunjungan') || label.textContent.includes('tujuan kunjungan'))) {
                        if (radio.checked && radio.value === 'Kasus') {
                            kasusSelected = true;
                        }
                    }
                });
                
                // If Kasus is selected, don't hide the jenis kasus section
                if (kasusSelected) {
                    console.log('Kasus is selected, not hiding jenis kasus section');
                    return;
                }
                
                const allFormGroups = document.querySelectorAll('.form-group');
                console.log('Found', allFormGroups.length, 'form groups');
                ...
                allFormGroups.forEach(function(group) {
                    ...
                    if (label) {
                        ...
                        // Hide jenis kasus section (the main radio button group)
                        if (labelText.includes('jenis kasus') && !labelText.includes('bacterial') && 
                            !labelText.includes('virus') && !labelText.includes('parasit') && 
                            !labelText.includes('jamur') && !labelText.includes('lain-lain') && 
                            !labelText.includes('lain_lain')) {
                            
                            console.log('Hiding jenis kasus section:', labelText);
                            
                            // Add multiple classes for maximum hiding
                            group.classList.add('jenis-kasus-section');
                        ...
                        // Hide jenis kasus dropdown fields (bacterial, virus, parasit, jamur, lain-lain)
                        if ((labelText.includes('bacterial') || labelText.includes('virus') || 
                            ...
                            console.log('Hiding jenis kasus dropdown field:', labelText);
                ...
                console.log('hideJenisKasusFieldsImmediately completed');
            }
            ...
            setTimeout(hideJenisKasusFieldsImmediately, 1000);
            setTimeout(hideJenisKasusFieldsImmediately, 2000);
            
            // Also run on window load
            window.addEventListener('load', hideJenisKasusFieldsImmediately);
            
            // Additional aggressive hiding - but only if no tujuan kunjungan is selected or if monitoring is selected
            setInterval(function() {
                // Check if any tujuan kunjungan radio button is selected
                const tujuanKunjunganRadios = document.querySelectorAll('input[type="radio"]');
                let kasusSelected = false;
                ...
                // Only hide if Kasus is not selected
                if (!kasusSelected) {
                    hideJenisKasusFieldsImmediately();
                }
            }, 1000);
        ...
        // Additional script to ensure hiding works - but only on initial load
        document.addEventListener('DOMContentLoaded', function() {
            // Force hide all jenis kasus elements only on initial load
            function forceHideAllJenisKasus() {
                // Check if Kasus is selected in tujuan kunjungan
                const tujuanKunjunganRadios = document.querySelectorAll('input[type="radio"]');
                let kasusSelected = false;
                ...
                // If Kasus is selected, don't hide the jenis kasus section
                if (kasusSelected) {
                    console.log('Kasus is selected, not force hiding jenis kasus section');
                    return;
                }
                
                const allElements = document.querySelectorAll('*');
                allElements.forEach(function(element) {
                    if (element.textContent && element.textContent.toLowerCase().includes('jenis kasus')) {
                        const formGroup = element.closest('.form-group');
                        if (formGroup) {
                            formGroup.style.display = 'none';
                            formGroup.style.visibility = 'hidden';
                            formGroup.style.opacity = '0';
                            formGroup.style.height = '0';
                            formGroup.style.overflow = 'hidden';
                            formGroup.style.margin = '0';
                            formGroup.style.padding = '0';
                            formGroup.style.position = 'absolute';
                            formGroup.style.left = '-9999px';
                            formGroup.style.top = '-9999px';
            ...
            // Run only on initial load, not continuously
            forceHideAllJenisKasus();
            setTimeout(forceHideAllJenisKasus, 100);
            setTimeout(forceHideAllJenisKasus, 500);
        });
    </script>
</head>
<body>
        ...
        <div id="initialQuestions">
            <?php if (!empty($questions)): ?>
                <?php foreach ($questions as $q): ?>
                    <?php 
                    // Check if this is a jenis kasus question and hide it by default
                    $isJenisKasus = false;
                    $isJenisKasusDropdown = false;
                    
                    if (isset($q['field_name']) && $q['field_name'] == 'jenis_kasus') {
                        $isJenisKasus = true;
                    }
                    if (isset($q['question_text']) && stripos($q['question_text'], 'jenis kasus') !== false) {
                        $isJenisKasus = true;
                    }
                    
                    // Check if this is a jenis kasus dropdown field (bacterial, virus, parasit, jamur, lain-lain)
                    if (isset($q['question_text'])) {
                        $questionText = strtolower($q['question_text']);
                        if (strpos($questionText, 'bacterial') !== false || 
                            strpos($questionText, 'virus') !== false || 
                            strpos($questionText, 'parasit') !== false || 
                            strpos($questionText, 'jamur') !== false || 
                            strpos($questionText, 'lain-lain') !== false || 
                            strpos($questionText, 'lain_lain') !== false) {
                            $isJenisKasusDropdown = true;
                        }
                    }
                    ...
                    $hideStyle = '';
                    if ($isJenisKasus) {
                        $hideClass = 'jenis-kasus-section jenis-kasus-hidden';
                        ...
                        $hideClass = 'jenis-kasus-dropdown jenis-kasus-hidden';
                        $hideStyle = 'style="display: none; visibility: hidden; opacity: 0; height: 0; overflow: hidden; margin: 0; padding: 0; position: absolute; left: -9999px; top: -9999px; z-index: -9999;"';
                    }
                    ?>
        ...
        <button type="submit" class="btn-submit" id="submitBtn" data-visiting-type="">Next</button>
         ...
         function toggleJenisKasus(tujuanKunjungan) {
               ...
               if (jenisKasusGroup) {
                   if (tujuanKunjungan === 'Monitoring') {
                       ...
                       jenisKasusGroup.classList.add('jenis-kasus-section', 'jenis-kasus-hidden');
                       jenisKasusGroup.style.display = 'none';
                       jenisKasusGroup.style.visibility = 'hidden';
                       jenisKasusGroup.style.opacity = '0';
                       jenisKasusGroup.style.height = '0';
                       jenisKasusGroup.style.overflow = 'hidden';
                       jenisKasusGroup.style.margin = '0';
                       jenisKasusGroup.style.padding = '0';
                       jenisKasusGroup.style.position = 'absolute';
                       jenisKasusGroup.style.left = '-9999px';
                       jenisKasusGroup.style.top = '-9999px';
                       jenisKasusGroup.style.zIndex = '-9999';
                       ...
                       jenisKasusGroup.classList.remove('jenis-kasus-section', 'jenis-kasus-hidden');
                       jenisKasusGroup.style.display = 'block';
                       jenisKasusGroup.style.visibility = 'visible';
                       jenisKasusGroup.style.opacity = '1';
                       jenisKasusGroup.style.height = 'auto';
                       jenisKasusGroup.style.overflow = 'visible';
                       jenisKasusGroup.style.margin = '';
                       jenisKasusGroup.style.padding = '';
                       jenisKasusGroup.style.position = '';
                       jenisKasusGroup.style.left = '';
                       jenisKasusGroup.style.top = '';
                       jenisKasusGroup.style.zIndex = '';
                       ...
                       // Hide all jenis kasus dropdown fields initially when showing jenis kasus
                       hideAllJenisKasusFields();
                   }
               } else {
               ...
               if (dynamicJenisKasusGroup) {
                   if (tujuanKunjungan === 'Monitoring') {
                       // Hide Jenis Kasus question when Monitoring is selected
                       dynamicJenisKasusGroup.classList.add('jenis-kasus-section', 'jenis-kasus-hidden');
                       dynamicJenisKasusGroup.style.display = 'none';
                       dynamicJenisKasusGroup.style.visibility = 'hidden';
                       dynamicJenisKasusGroup.style.opacity = '0';
                       dynamicJenisKasusGroup.style.height = '0';
                       dynamicJenisKasusGroup.style.overflow = 'hidden';
                       dynamicJenisKasusGroup.style.margin = '0';
                       dynamicJenisKasusGroup.style.padding = '0';
                       dynamicJenisKasusGroup.style.position = 'absolute';
                       dynamicJenisKasusGroup.style.left = '-9999px';
                       dynamicJenisKasusGroup.style.top = '-9999px';
                       dynamicJenisKasusGroup.style.zIndex = '-9999';
                       ...
                       // Show Jenis Kasus question when Kasus is selected
                       dynamicJenisKasusGroup.classList.remove('jenis-kasus-section', 'jenis-kasus-hidden');
                       dynamicJenisKasusGroup.style.display = 'block';
                       dynamicJenisKasusGroup.style.visibility = 'visible';
                       dynamicJenisKasusGroup.style.opacity = '1';
                       dynamicJenisKasusGroup.style.height = 'auto';
                       dynamicJenisKasusGroup.style.overflow = 'visible';
                       dynamicJenisKasusGroup.style.margin = '';
                       dynamicJenisKasusGroup.style.padding = '';
                       dynamicJenisKasusGroup.style.position = '';
                       dynamicJenisKasusGroup.style.left = '';
                       dynamicJenisKasusGroup.style.top = '';
                       dynamicJenisKasusGroup.style.zIndex = '';
           ...
           function hideAllJenisKasusFields() {
                console.log('Executing hideAllJenisKasusFields...');
                ...
                formGroups.forEach(function(group) {
                    ...
                    if (label) {
                        ...
                        if (labelText.includes('bacterial') || labelText.includes('virus') || 
                            ...
                            group.classList.add('jenis-kasus-dropdown', 'jenis-kasus-hidden');
                            group.style.display = 'none';
                            group.style.visibility = 'hidden';
                            group.style.opacity = '0';
                            group.style.height = '0';
                ...
                formGroups.forEach(function(group) {
                    ...
                    if (label) {
                        ...
                        if (labelText.includes(targetFieldName.toLowerCase()) || 
                            ...
                            console.log('Showing field for:', targetFieldName, 'Label:', labelText);
                            
                            // Remove the hiding class and show the field
                            group.classList.remove('jenis-kasus-dropdown', 'jenis-kasus-hidden');
                ...
                dynamicFormGroups.forEach(function(group) {
                    ...
                    if (label) {
                        ...
                        if (labelText.includes(targetFieldName.toLowerCase()) || 
                            ...
                            console.log('Showing dynamic field for:', targetFieldName, 'Label:', labelText);
                         ...
                         radio.addEventListener('change', function() {
                             ...
                             if (jenisKasusGroup) {
                                  if (selectedValue === 'Monitoring') {
                                      jenisKasusGroup.classList.add('jenis-kasus-section');
                                      jenisKasusGroup.style.display = 'none';
                                      jenisKasusGroup.style.visibility = 'hidden';
                                      jenisKasusGroup.style.opacity = '0';
                                      jenisKasusGroup.style.height = '0';
                                      ...
                                      // Also hide all jenis kasus dropdown fields
                                      hideAllJenisKasusFields();
                                  } else if (selectedValue === 'Kasus') {
                                      jenisKasusGroup.classList.remove('jenis-kasus-section');
                                      jenisKasusGroup.style.display = 'block';
                                      jenisKasusGroup.style.visibility = 'visible';
                                      jenisKasusGroup.style.opacity = '1';
                                      jenisKasusGroup.style.height = 'auto';
         ...
         function addJenisKasusFieldsLogic() {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Hide all jenis kasus fields and sections on page load first
                setTimeout(function() {
                    hideJenisKasusDropdownFields();
                    forceHideJenisKasusFields();
                    hideJenisKasusSection();
                    console.log('Initial hiding of jenis kasus fields and sections completed');
                }, 100);
                ...
                setTimeout(function() {
                    ...
                    forceHideJenisKasusFields();
                    hideJenisKasusSection();
                    console.log('Additional hiding of jenis kasus fields and sections completed');
                }, 500);
                ...
                setTimeout(function() {
                    ...
                    forceHideJenisKasusFields();
                    hideJenisKasusSection();
                    console.log('Final hiding of jenis kasus fields and sections completed');
                }, 1000);
            });
           ...
           function initializeJenisKasusFieldsLogic() {
            ...
            function hideJenisKasusSection() {
                console.log('Executing hideJenisKasusSection...');
                ...
                // Find all form groups in both initial and dynamic questions
                const allFormGroups = document.querySelectorAll('.form-group');
                let hiddenCount = 0;
                ...
                allFormGroups.forEach(function(group) {
                    ...
                    if (label) {
                        ...
                        // Check for jenis kasus section (the main radio button group)
                        if (labelText.includes('jenis kasus') && !labelText.includes('bacterial') && 
                            !labelText.includes('virus') && !labelText.includes('parasit') && 
                            !labelText.includes('jamur') && !labelText.includes('lain-lain') && 
                            !labelText.includes('lain_lain')) {
                            ...
                            hiddenCount++;
                            console.log('Hidden jenis kasus section:', labelText);
                        }
                    }
                });
                
                console.log('Hidden', hiddenCount, 'jenis kasus sections');
            }
            ...
            function forceHideJenisKasusFields() {
                ...
                // If Kasus is selected, don't hide the jenis kasus section
                if (kasusSelected) {
                    console.log('Kasus is selected, not force hiding jenis kasus fields');
                    return;
                }
