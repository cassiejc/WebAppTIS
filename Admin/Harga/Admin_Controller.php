   ...
   // Gantikan fungsi edit_farm() yang lama dengan ini
    public function edit_farm($id)
    {
        ...
        $data = [
            ...
            'form_action'       => base_url('admin_controller/update_farm')
        ];
    ...
    public function edit_user($id_user)
    {
        ...
        $data = [
            ...
            'existing_data'     => $existing_data,
            'questions_kategori'=> $questions,
            'form_action'       => base_url('admin_controller/update_user')
        ];
    ...
    public function update_user()
    {
        ...
        $update_data = [];
        foreach ($questions as $q) {
            if ($q['type'] !== 'text_readonly') { // Lewati field readonly
                $field = $q['field_name'];
                $input_name = 'q' . $q['questions_id'];
                $jawaban = $this->input->post($input_name);
                $update_data[$field] = $jawaban;
        ...
        $new_area_id = $update_data['master_area_id'];
        $new_sub_area_id = $update_data['master_sub_area_id'];
        $start_date_str = $update_data['start_date'];
    ...
    public function add_harga()
    {
        $data = [
            ...
            'form_action' => base_url('admin_controller/create_harga_action')
            ] ;
    ...
    private function _process_options(&$questions_kategori, $kategori, $user) {
        switch($kategori) {
            ...
            case 'User':
                foreach($questions_kategori as &$q) { // <-- Pastikan baris foreach ini ada dan benar
                    if ($q['type'] === 'select') {
    ...
    private function _delete_data($kategori, $id, $user, $existing_data) {
        switch($kategori) {
            ...
            case 'Harga':
                $this->M_master_harga->delete_harga($id);
                return $this->db->affected_rows() > 0;
