...
class Admin_Controller extends CI_Controller {
    ...
    public function edit_kontributor_harga($id_user)
    {
        ...
        $questions = $this->M_Questions->get_questions_by_page('master_user_harga');
        $is_active = $this->M_edit_user->is_user_contribution_active($id_user);

        // "Suntikkan" nilai agar checkbox bisa tercentang otomatis
        // Nama field 'is_kontributor' harus sama dengan 'field_name' di tabel questions
        $existing_user_data['is_kontributor'] = $is_active ? '1' : '0';

        $data = [
            'title'               => 'CP APPS',
            'page_title'          => 'Edit Status Kontributor: ' . $existing_user_data['username'],
            'kategori_selected'   => 'Kontributor Harga',
            'edit_id'             => $id_user,
            'existing_data'       => $existing_user_data,
            'questions_kategori'  => $questions,
            'form_action'         => site_url('Admin_Controller/update_kontributor_harga')
        ];

        $this->load->view('templates/dash_h', $data);
        $this->load->view('form_admin_update_view', $data); // Menggunakan view form yang sudah ada
        $this->load->view('templates/dash_f', $data);
    }
    ...
    public function update_kontributor_harga()
    {
        ...
        // Dapatkan ID pertanyaan dari database untuk field 'is_kontributor'
        $question = $this->db->get_where('questions', ['page' => 'master_user_harga', 'field_name' => 'is_kontributor'])->row_array();
        $input_name = 'q' . $question['questions_id'];

        $is_selected = $this->input->post($input_name) ? true : false;
        $is_currently_active = $this->M_edit_user->is_user_contribution_active($id_user);

        $this->db->trans_start();

        if ($is_selected && !$is_currently_active) {
            $this->M_edit_user->activate_user_contribution($id_user);
        } 
        elseif (!$is_selected && $is_currently_active) {
            $this->M_edit_user->deactivate_user_contribution($id_user);
        }
    ...
    /**
     * Fungsi Pemicu Utama untuk menghitung ulang SEMUA harga.
     * Ini akan memanggil setiap proses harian, bulanan, dan tahunan.
     */
    public function hitung_ulang_semua_harga()
    {
        try {
            ...
            if ($id_telur) {
                $this->M_master_harga->proses_rata_rata_harian_telur_layer($id_telur);
                $this->M_master_harga->hitung_rata_rata_bulanan_telur_layer($id_telur, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_telur_layer($id_telur, $tahun_sekarang);
            }

            // --- 2. Proses Harga Jagung ---
            $id_jagung = $this->_get_id_by_nama_harga('Average Harga Jagung');
            if ($id_jagung) {
                $this->M_master_harga->proses_rata_rata_harian_jagung($id_jagung);
                $this->M_master_harga->hitung_rata_rata_bulanan_jagung($id_jagung, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_jagung($id_jagung, $tahun_sekarang);
            }

            // --- 3. Proses Harga Katul ---
            $id_katul = $this->_get_id_by_nama_harga('Average Harga Katul');
            if ($id_katul) {
                // Asumsi fungsi-fungsi ini sudah Anda buat
                $this->M_master_harga->proses_rata_rata_harian_katul($id_katul);
                $this->M_master_harga->hitung_rata_rata_bulanan_katul($id_katul, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_katul($id_katul, $tahun_sekarang);
            }

            // --- 4. Proses Harga Konsentrat Layer (Turunan) ---
            $id_konsentrat = $this->_get_id_by_nama_harga('Average Harga Konsentrat Layer');
            if ($id_konsentrat) {
                $this->M_master_harga->proses_harga_konsentrat_layer_harian($id_konsentrat);
                $this->M_master_harga->hitung_rata_rata_bulanan_konsentrat_layer($id_konsentrat, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_konsentrat_layer($id_konsentrat, $tahun_sekarang);
            }
            
            // --- 5. Proses HPP Konsentrat Layer (Turunan) ---
            $id_hpp_konsentrat = $this->_get_id_by_nama_harga('Average HPP Konsentrat Layer');
            if ($id_hpp_konsentrat) {
                $this->M_master_harga->proses_hpp_konsentrat_layer_harian($id_hpp_konsentrat);
                $this->M_master_harga->hitung_rata_rata_bulanan_hpp_konsentrat_layer($id_hpp_konsentrat, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_hpp_konsentrat_layer($id_hpp_konsentrat, $tahun_sekarang);
            }

            // --- 6. Proses HPP Komplit Layer (Turunan) ---
            $id_hpp_komplit = $this->_get_id_by_nama_harga('Average HPP Komplit Layer');
            if ($id_hpp_komplit) {
                $this->M_master_harga->proses_hpp_komplit_layer_harian($id_hpp_komplit);
                $this->M_master_harga->hitung_rata_rata_bulanan_hpp_komplit_layer($id_hpp_komplit, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_hpp_komplit_layer($id_hpp_komplit, $tahun_sekarang);
            }

            $id_cost_broiler = $this->_get_id_by_nama_harga('Average Cost Komplit Broiler');
            if ($id_cost_broiler) {
                $this->M_master_harga->proses_harga_komplit_broiler_harian($id_cost_broiler);
                $this->M_master_harga->hitung_rata_rata_bulanan_cost_komplit_broiler($id_cost_broiler, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_cost_komplit_broiler($id_cost_broiler, $tahun_sekarang);
            }

            $id_hpp_broiler = $this->_get_id_by_nama_harga('Average HPP Broiler');
            if ($id_hpp_broiler) {
                $this->M_master_harga->proses_hpp_broiler_harian($id_hpp_broiler);
                $this->M_master_harga->hitung_rata_rata_bulanan_hpp_broiler($id_hpp_broiler, $tahun_sekarang, $bulan_sekarang);
                $this->M_master_harga->hitung_rata_rata_tahunan_hpp_broiler($id_hpp_broiler, $tahun_sekarang);
            }

            // (Tambahkan blok lain untuk harga-harga lainnya di sini)
    ...
    private function _get_data_list($kategori, $user) {
        ...
        switch($kategori) {
            ...
            case 'Farm':
                ...
                return [
                    'data' => $this->db->get()->result_array(),
                    'headers' => ['Nama Farm', 'Nama Peternak', 'Area', 'Sub Area', 'Tipe Ternak', 'Alamat', 'Kapasitas'],
                    
                    // 3. Mengganti fields dari ID menjadi nama yang sesuai
                    'fields' => ['nama_farm', 'nama_peternak', 'nama_area', 'nama_sub_area', 'tipe_ternak', 'alamat_farm', 'kapasitas_farm'],
