...
class Admin_Controller extends CI_Controller {
    ...
    public function target($id = null) {
        ...
        $this->edit_target($id); // Panggil method edit_target
    }
    ...
    public function user()
    {
        ...
        $data['users'] = $this->M_edit_user->get_all_users();

        $this->load->view('templates/dash_h', $data);
        $this->load->view('user_list_view', $data); // View baru untuk daftar user
        $this->load->view('templates/dash_f', $data);
    }
    ...
    public function edit_user($id_user)
    {
        ...
        // Ambil data area dan sub-area untuk dropdown
        ...
        $this->load->view('templates/dash_h', $data);
        $this->load->view('user_edit_view', $data); // View baru untuk form edit
        $this->load->view('templates/dash_f', $data);
    }

    // GANTI FUNGSI UPDATE_USER ANDA DENGAN KODE LENGKAP INI
    public function update_user()
    {
        ...
        // Cek apakah ada perubahan pada area atau sub-area
        $areaHasChanged = ($currentUser['master_area_id'] != $new_area_id || $currentUser['master_sub_area_id'] != $new_sub_area_id);
        
        // --- Memulai Database Transaction ---
        $this->db->trans_start();

        // JIKA ADA PERUBAHAN AREA, jalankan logika history
        if ($areaHasChanged) {
            // 1. "Tutup" riwayat yang lama
            // Tanggal akhir adalah kemarin (sehari sebelum hari ini)
            $end_date_str = date('Y-m-d', strtotime('-1 day', strtotime($start_date_str)));
            $this->M_edit_user->close_current_area_history($id_user, $end_date_str);

            // 2. "Buka" riwayat yang baru
            // Tanggal mulai adalah hari ini, tanggal akhir adalah "tak terhingga"
            $new_history_data = [
                'id_user' => $id_user,
        ...
        // 3. Tetap update data lain di tabel utama (z_master_user)
        // Ini untuk kemudahan mengambil data user yang aktif saat ini
        $data_user = [
            'master_area_id' => $new_area_id,
            'master_sub_area_id' => $new_sub_area_id,
    ...
    // FUNGSI BARU untuk menampilkan form edit farm
    // GANTI FUNGSI edit_farm DENGAN VERSI DINAMIS INI
    public function edit_farm($id)
    {
        $data['page_title'] = 'Edit Farm';
        
        // 1. Ambil struktur form dari database (TIDAK LAGI HARDCODE)
        $data['questions'] = $this->M_Questions->get_questions_by_page('master_farm_edit');
        
        // 2. Ambil data farm yang sudah ada untuk mengisi nilai form
        $data['existing_data'] = $this->M_master_farm->get_farm_by_id($id);

        // 3. Atur nilai default untuk tanggal mulai
        if (!isset($data['existing_data']['start_date'])) {
            $data['existing_data']['start_date'] = date('Y-m-d');
        }

        if (empty($data['existing_data'])) {
            show_404();
        }

        // 4. Muat view dinamis yang baru
        $this->load->view('templates/dash_h', $data);
        $this->load->view('farm_edit_view', $data); // View ini akan kita buat dinamis
        $this->load->view('templates/dash_f', $data);
    }

    // FUNGSI BARU untuk memproses update farm dengan logika log
    public function update_farm()
    {
        ...
        // Ambil data farm saat ini untuk perbandingan
        $currentFarm = $this->M_master_farm->get_farm_by_id($master_farm_id);

        // Cek apakah nilai kapasitas berubah
        $capacityHasChanged = ($currentFarm['kapasitas_farm'] != $new_capacity);
        
        $this->db->trans_start();

        // JIKA KAPASITAS BERUBAH, jalankan logika history
        ...
        // Tetap update data lain di tabel utama (master_farm)
        $data_farm = [
            'nama_farm'      => $this->input->post('nama_farm'),
            'kapasitas_farm' => $new_capacity
            // Tambahkan field lain yang ingin di-update dari form
        ];
