...
class M_master_harga extends CI_Model {
    ...
    public function calculate_and_save_average_history($id_harga, $date=null)
    {
        // 1. SELECT: Pilih nilai rata-rata dari kolom harga yang diinginkan
        $this->db->select('AVG(vpl.layer_harga_jual_telur) as rata_rata');
        $this->db->from('visiting_p_layer vpl');
        ...
        $this->db->where('DATE(vpl.waktu_kunjungan)', $calculation_Date);
        
        // Ambil hasil perhitungan
        $result = $this->db->get()->row();

        // 4. SIMPAN: Jika ada hasil, simpan ke tabel harga_rata_rata_history
        if ($result && $result->rata_rata !== null) {
            $new_average = $result->rata_rata;

            $this->db->trans_start();
            $this->update_harga($id_harga, ['nilai_harga' => $new_average]);
            $this->db->delete('history_harga_telur_layer', [
                'id_harga' => $id_harga,
            ...
            $this->db->insert('history_harga_telur_layer', $history_data);
