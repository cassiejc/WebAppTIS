...
class Admin_Controller extends CI_Controller {
    ...
    public function hitung_ulang_harga_telur_layer($id_harga = null)
    {
        ...
        $selected_date = $this->input->post('calculation_date');
        if (!$selected_date) {
            $this->session->set_flashdata('error', 'Tanggal perhitungan wajib diisi.');
            redirect('Admin_Controller/list_data/harga');
            return;
        }

        $success = $this->M_master_harga->calculate_and_save_average_history($id_harga, $selected_date);

        if ($success) {
            $this->session->set_flashdata('success', 'Harga rata-rata telur layer untuk tanggal' . $selected_date . ' berhasil dihitung ulang dan disimpan.');
        } else {
            $this->session->set_flashdata('error', 'Gagal menghitung harga. Tidak ada data kunjungan pada tanggal' . $selected_date . ' atau terjadi kesalahan lainnya.');
        }
