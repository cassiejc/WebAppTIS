...
class Dashboard_new extends MY_Controller
{
    ...
    public function visual_data_kunjungan()
    {
        ...
        // Mengubah filter menjadi rentang tanggal (lebih baik untuk log)
        // Ambil dari GET agar pagination berfungsi
        $start_date = $this->input->get('start_date') ?? date('Y-m-01');
        $end_date = $this->input->get('end_date') ?? date('Y-m-t');

        // Untuk kompatibilitas dengan fungsi lama yang butuh bulan & tahun
        // Ini mengambil bulan dan tahun dari tanggal awal yang dipilih
        $selected_month = date('m', strtotime($start_date));
        $selected_year = date('Y', strtotime($start_date));


        // --- DATA UNTUK LAPORAN LAMA (Performa, Area, Pie Chart) ---
        ...
        // Mengambil semua data mentah untuk pie chart
        ...
        // Gabungkan semua data mentah menjadi satu array
        ...
        // Proses pengelompokan baru untuk Pie Chart
        $grouped_counts = [
            'Agen/Subagen/Lainnya' => 0,
            'Demoplot DOC' => 0,
            'Others' => 0,
        ];
        foreach ($combined_data as $item) {
            $kategori_asal = $item['kategori'];
            $jumlah = (int)$item['jumlah_visit'];
            switch ($kategori_asal) {
                case 'Agen':
                case 'Sub Agen':
                case 'Kantor':
                case 'Kemitraan':
                    $grouped_counts['Agen/Subagen/Lainnya'] += $jumlah;
                    break;
                case 'Grower':
                    $grouped_counts['Demoplot DOC'] += $jumlah;
                    break;
                case 'Arap':
                case 'Bebek Pedaging':
                case 'Bebek Petelur':
                case 'Puyuh':
                case 'Sample':
                    $grouped_counts['Others'] += $jumlah;
                    break;
                default:
                    if (!isset($grouped_counts[$kategori_asal])) {
                        $grouped_counts[$kategori_asal] = 0;
                    }
                    $grouped_counts[$kategori_asal] += $jumlah;
                    break;
            }
        }
        $final_breakdown_raw = [];
        foreach ($grouped_counts as $kategori => $jumlah) {
            if ($jumlah > 0) {
                $final_breakdown_raw[] = ['kategori' => $kategori, 'jumlah_visit' => $jumlah];
            }
        }
        $grand_total = array_sum(array_column($final_breakdown_raw, 'jumlah_visit'));
        $final_breakdown = [];
        if ($grand_total > 0) {
            foreach ($final_breakdown_raw as $item) {
                $final_breakdown[] = [
        ...
        $data['breakdown_chart_labels'] = json_encode(array_column($final_breakdown, 'kategori'));
        $data['breakdown_chart_values'] = json_encode(array_column($final_breakdown, 'persentase'));


        // --- DATA BARU UNTUK LOG KUNJUNGAN ---
        $this->load->library('pagination');
        $filters = ['start_date' => $start_date, 'end_date' => $end_date];

        $config['base_url'] = site_url('Dashboard_new/visual_data_kunjungan');
        $config['total_rows'] = $this->visual->count_all_visits_log($filters);
        $config['per_page'] = 25;
        ...
        $data['visits_log'] = $this->visual->get_all_visits_log($filters, $config['per_page'], $offset);
        $data['pagination_links'] = $this->pagination->create_links();
        $data['filters'] = $filters;


        // Kirim data filter lama ke view untuk kompatibilitas
