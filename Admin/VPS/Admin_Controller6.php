...
class Admin_Controller extends CI_Controller {
    ...
    public function pakan($id = null) {
        ...
        // GANTI 'master_pakan_tambah' DENGAN NAMA PAGE YANG ANDA TEMUKAN DI LANGKAH 1
        $page = 'master_pakan'; 
        $model = 'M_master_pakan';

        // 1. Mengambil data pakan yang ada untuk diedit
        $existing_data = $this->{$model}->get_pakan_by_id($id);
        if (!$existing_data) {
            show_404();
        }

        // 2. Logika saat form disubmit (method POST)
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            $questions = $this->M_Questions->get_questions_by_page($page);
            $post_data = [];

            foreach ($questions as $q) {
                $post_data[$q['field_name']] = $this->input->post('q' . $q['questions_id']);
            }
            
            // 3. Menentukan nama pakan yang akan disimpan
            $final_pakan_data = [];
            $final_pakan_data['tipe_ternak'] = $post_data['tipe_ternak'];
            
            // Ganti 'jenis_pakan_layer', 'nama_pakan_cp', dll, dengan field_name Anda yang sebenarnya
            if ($post_data['tipe_ternak'] === 'Layer') {
                $final_pakan_data['jenis_pakan'] = $post_data['jenis_pakan_layer']; // Sesuaikan field_name
                if ($post_data['jenis_pakan_layer'] === 'CP') {
                    $final_pakan_data['nama_pakan'] = $post_data['layer_pilihan_pakan_cp']; // Sesuaikan field_name
                } else {
                    $final_pakan_data['nama_pakan'] = $post_data['layer_pilihan_pakan_lain']; // Sesuaikan field_name
                }
            } else {
                $final_pakan_data['jenis_pakan'] = null;
                $final_pakan_data['nama_pakan'] = $post_data['nama_pakan']; // Sesuaikan field_name
            }

            // Lakukan update ke database
            $this->db->where('master_pakan_id', $id)->update('master_pakan', $final_pakan_data);

            // Update juga tabel options jika nama pakan berubah
            if (isset($existing_data['nama_pakan']) && $existing_data['nama_pakan'] != $final_pakan_data['nama_pakan']) {
                $this->_update_pakan_options($existing_data, $final_pakan_data);
            }

            $this->session->set_flashdata('success', 'Data pakan berhasil diupdate!');
            redirect('Admin_Controller/list_data/pakan');
            return;
        }

        // 4. Logika saat menampilkan form (method GET)
        // Mengambil struktur pertanyaan dari form TAMBAH BARU
        $questions = $this->M_Questions->get_questions_by_page($page);

        // Mengisi options untuk dropdown pakan CP dan Non CP
        // Pastikan Anda memiliki fungsi ini di M_master_pakan.php
        $pakan_cp_options = $this->{$model}->get_pakan_options_by_jenis('CP');
        $pakan_non_cp_options = $this->{$model}->get_pakan_options_by_jenis('Non CP');

        // Suntikkan OPSI ke dalam array questions
        foreach ($questions as &$q) {
            if ($q['field_name'] === 'tipe_ternak') {
                $q['options'] = $this->db->get_where('options', ['questions_id' => $q['questions_id']])->result_array();
            }
            // Ganti field_name di bawah ini dengan yang Anda temukan
            if ($q['field_name'] === 'jenis_pakan_layer') {
                $q['options'] = [['option_text' => 'CP'], ['option_text' => 'Non CP']];
            }
            if ($q['field_name'] === 'layer_pilihan_pakan_cp') {
                $q['options'] = $pakan_cp_options;
            }
            if ($q['field_name'] === 'layer_pilihan_pakan_lain') {
                $q['options'] = $pakan_non_cp_options;
            }
        }
    ...
    public function hitung_ulang_harga_telur_layer($id_harga = null)
    {
        ...
        $success = $this->M_master_harga->calculate_and_update_average_telur($id_harga);

        if ($success) {
            $this->session->set_flashdata('success', 'Harga rata-rata telur layer berhasil dihitung ulang!');
        } else {
            $this->session->set_flashdata('error', 'Gagal menghitung ulang harga. Tidak ada data valid yang ditemukan.');
        }
