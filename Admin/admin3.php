...
class Admin_Controller extends CI_Controller {
    ...
    public function target($id = null) {
        ...
        $this->_handle_update_form('Target', 'master_target', $id);
    }
            ...
            case 'Peternak':
                ...
                case 'Target':
                    $this->db->where('id_target', $id)
                        ->update('master_target', $update_data);  // atau nama tabel target yang sebenarnya
                        
                    // Update options if name changed (jika diperlukan)
                    if (isset($existing_data['username']) && $existing_data['username'] != $update_data['username']) {
                        $this->_update_options('master_target', $existing_data['username'], $update_data['username'], 0); // master_sub_area_id = 0 atau sesuai kebutuhan
                    }
    ...
    private function _get_data_list($kategori, $user) {
        switch($kategori) {
            ...
            case 'Target':
                ...
                return [
                    'data' => $query->result_array(),
                    'headers' => ['ID Target', 'Username', 'Target'],
                    'fields' => ['id_target', 'username', 'target'],
                    'primary_key' => 'id_target',
                    'display_field' => 'username'
                ];
    ...
    private function _delete_data($kategori, $id, $user, $existing_data) {
        switch($kategori) {
            ...
            case 'Target':
                // Targets are not deletable in this implementation
                if (isset($existing_data['username'])) {
                        $this->_delete_options('master_target', $existing_data['username'], 0); // master_sub_area_id = 0 atau sesuai kebutuhan
                    }
                    // Delete main data
                    $this->db->where('id_target', $id)
                            ->delete('master_target');  // atau nama tabel target yang sebenarnya
                    return $this->db->affected_rows() > 0;                
            default:
                return false;
        }
