...
<body>
    ...
    <?php
        // Logika ini akan mengambil URL action dari controller jika ada (khusus untuk Target),
        // atau membiarkannya kosong untuk form generik lainnya.
        $action_url = isset($form_action) ? $form_action : ''; 
    ?>
    <form method="post" action="<?= $action_url; ?>" class="form-container" id="mainForm">

    <input type="hidden" name="edit_id" value="<?= htmlspecialchars($edit_id) ?>">            <?php if (!empty($questions_kategori)): ?>
                            ...
                            <?php if ($q['field_name'] == 'jenis_peternak'): ?>
                                <select name="q<?= $q['questions_id'] ?>" 
                                    ...
                                    <?php if (!empty($q['options'])): ?>
                                        <?php foreach ($q['options'] as $opt): ?>
                                            <option value="<?= $opt['option_text'] ?>" <?= ($current_value == $opt['option_text']) ? 'selected' : '' ?>>
                                                <?= $opt['option_text'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                ...
                <div class="mt-4">
                    <a href="<?= base_url('Dashboard_new/index') ?>" class="btn btn-secondary px-4 py-2">Batal</a>
                    <button type="submit" name="submit_form" value="1" class="btn btn-primary px-4 py-2">Update</button>
                </div>
            <?php else: ?>
                ...
                <div class="mt-4">
                    <a href="<?= base_url('Dashboard_new/index') ?>" class="btn btn-secondary px-4 py-2">Kembali</a>
                </div>
            <?php endif; ?>
        </form>
    </div>
