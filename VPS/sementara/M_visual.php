...
class M_Visual extends CI_Model {
    ...
    /**
     * Mengambil data detail satu kunjungan spesifik dari tabel visiting_p_grower.
     * --- DIMODIFIKASI: Menambahkan JOIN ke master_strain_standard ---
     */
    public function get_grower_visit_detail($farm_name, $waktu_kunjungan)
    {
        // Menambahkan alias 't' untuk tabel visit dan 'mss' untuk tabel standard
        $this->db->select("
            t.nama_farm,
            t.efektif_terisi_pedaging,
            t.strain_pedaging,
            DATE_FORMAT(t.tanggal_chick_in_pedaging, '%d %M %Y') as tanggal_chick_in_pedaging_formatted,
            DATE_FORMAT(t.waktu_kunjungan, '%d %M %Y, %H:%i') as waktu_kunjungan_formatted,
            t.umur_pedaging,
            
            /* --- Data Aktual Kunjungan --- */
            t.pencapaian_berat_pedaging,
            t.keseragaman_pedaging,
            t.intake_pedaging,
            t.deplesi_pedaging,

            /* --- BARU: Data Standard dari Master Strain --- */
            /* Menggunakan COALESCE untuk menampilkan '-' jika data standard tidak ada */
            COALESCE(mss.berat_badan, '-') AS berat_badan_strain,
            COALESCE(mss.keseragaman, '-') AS keseragaman_strain,
            COALESCE(mss.konsumsi_pakan, '-') AS konsumsi_pakan_strain,
            COALESCE(mss.konsumsi_pakan_kumulatif, '-') AS konsumsi_pakan_kulmulatif_strain,
            COALESCE(mss.kematian_kumulatif, '-') AS kematian_kulmulatif_strain
        ");
        $this->db->from('visiting_p_grower t'); // Tambahkan alias 't'
        
        // BARU: Tambahkan LEFT JOIN
        $this->db->join(
            'master_strain_standard mss', 
            't.strain_pedaging = mss.nama_strain AND t.umur_pedaging = mss.umur', 
            'left'
        );

        $this->db->where('t.nama_farm', $farm_name); // Gunakan alias 't'
        $this->db->where('t.waktu_kunjungan', $waktu_kunjungan); // Gunakan alias 't'
        $this->db->limit(1);
