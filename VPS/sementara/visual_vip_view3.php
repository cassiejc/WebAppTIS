        ...
        <?php if ($user['group_user'] === 'administrator'): ?>
            <div class="card shadow-sm area-filter-container">
                <form method="post" action="<?= current_url(); ?>">
                    ...
                    <h5 class="mb-3">Filter Berdasarkan Area:</h5>

                    <div class="dropdown mb-2">
                        <button class="btn btn-outline-secondary dropdown-toggle w-100" type="button" id="areaDropdownButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="text-align: left;">
                            Pilih Area (0 dipilih)
                        </button>
                        
                        <div class="dropdown-menu area-dropdown-menu w-100" aria-labelledby="areaDropdownButton">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- BARU: Fungsi untuk update teks tombol dropdown ---
    function updateAreaButtonText() {
        const button = document.getElementById('areaDropdownButton');
        if (!button) return; // Keluar jika tombol tidak ada
        
        const checkedCheckboxes = document.querySelectorAll('.area-checkbox:checked');
        const totalCheckboxes = areaCheckboxes.length;

        if (checkedCheckboxes.length === 0) {
            button.innerText = 'Pilih Area (0 dipilih)';
        } else if (checkedCheckboxes.length === totalCheckboxes && totalCheckboxes > 0) {
            button.innerText = 'Pilih Area (Semua dipilih)';
        } else {
            button.innerText = 'Pilih Area (' + checkedCheckboxes.length + ' dipilih)';
        }
    }
    // --- AKHIR FUNGSI BARU ---

    // --- BARU: Mencegah dropdown tertutup saat diklik di dalamnya ---
    const dropdownMenu = document.querySelector('.area-dropdown-menu');
    if (dropdownMenu) {
        dropdownMenu.addEventListener('click', function(e) {
            // Hentikan event agar tidak "bubble up" ke parent dropdown
            e.stopPropagation();
        });
    }
    // --- AKHIR BLOK BARU ---
    ...
    if (selectAllCheckbox) {
        selectAllCheckbox.addEventListener('change', function() {
            ...
            updateAreaButtonText(); // <-- PANGGIL FUNGSI BARU
        });
        ...
        if(areaCheckboxes.length > 0) { // Pastikan ada checkbox sebelum cek
            areaCheckboxes.forEach(cb => {
                if (!cb.checked) {
            ...
            initialAllChecked = false; // Jika tidak ada checkbox, jangan centang "Pilih Semua"
        }
        selectAllCheckbox.checked = initialAllChecked;
        
        updateAreaButtonText(); // <-- PANGGIL FUNGSI BARU SAAT LOAD
    }
    
    // =======================================================
    // KODE AJAX ANDA UNTUK TABEL DIMULAI DARI SINI (TIDAK BERUBAH)
    // =======================================================
