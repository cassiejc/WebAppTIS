...
/**
    ...
    public function visual_kandang_kosong()
    {
        ...
        // --- PERUBAHAN ---
        // Baris ini dihapus karena checkbox sudah tidak ada
        // $tampilkan_gabungan = $this->input->post('tampilkan_gabungan') ? 1 : 0;
        // --- AKHIR PERUBAHAN ---

        // 2. Siapkan array filter
        $filters = [
            'tipe_ternak' => $selected_tipe_ternak, // Logika baru akan ada di Model
            'start_month' => $start_month,
            'end_month' => $end_month
            // --- PERUBAHAN: 'tampilkan_gabungan' dihapus dari array ---
        ];
        
        // 3. Panggil model (tidak berubah)
        $raw_data = $this->visual->get_monthly_vacancy_percentage($filters);
        
        // --- (Sisa kode chart pivot tidak berubah) ---
        $labels = []; $pivot_data = [];
        ...
        // --- PERUBAHAN ---
        // Baris ini dihapus
        // $data['tampilkan_gabungan'] = $tampilkan_gabungan;
        // --- AKHIR PERUBAHAN ---
    ...
    public function visual_harga_compare()
    {
        ...
        $selected_tahun = $this->input->get('tahun') ?: 'semua'; // BARU: Filter tahun

        $data['selected_komoditas1'] = $komoditas1_key;
        $data['selected_komoditas2'] = $komoditas2_key;
        $data['selected_tahun'] = $selected_tahun; // BARU: Kirim ke view
        ...
        foreach ($data1_raw as $row) {
            ...
            // BARU: Filter berdasarkan tahun yang dipilih
            if ($selected_tahun == 'semua' || $row['tahun'] == $selected_tahun) {
    ...
    {
        // Variabel $tahun diset ke NULL.
        // Ini akan diteruskan ke fungsi model yang membutuhkannya.
        $tahun = null; 
    ...
    // public function visual_vip_farms() // NAMA FUNGSI BARU
    // {
    //     // 1. Standard User & Title Setup
    //     $token = $this->session->userdata('token');
    //     $data['user'] = $this->dash->getUserInfo($token)->row_array();
    ...
    //     // 4. Load Views
    //     $this->load->view('templates/dash_h', $data);
    //     $this->load->view('visual_vip_view', $data); // NAMA VIEW BARU ANDA
    //     $this->load->view('templates/dash_f', $data);
    // }
    ...
    // Tambahkan fungsi ini ke dalam class Dashboard_new di controller Dashboard_new.php
...
public function visual_harga_gabungan()
    {
        ...
        // 2. Ambil Filter
        // 'active_tab' untuk mengingat tab mana yang terakhir dibuka
        $data['active_tab'] = $this->input->get('active_tab') ?: 'laporan'; 

        // --- Filter untuk Tab "Laporan Harga" ---
        // (Kita ganti 'komoditas' menjadi 'komoditas_laporan' agar tidak bentrok)
        ...
        // ==================================================================
        // 4. LOGIC UNTUK TAB "PERBANDINGAN HARGA" (dari visual_harga_compare)
        // ==================================================================
        $data['all_komoditas'] = [
            'telur' => 'Harga Telur Layer',
            'jagung' => 'Harga Jagung',
        ...
        // PENTING: Ganti nama variabel agar tidak bentrok dengan chart laporan
        ...
        // 5. Load View Gabungan
        $this->load->view('templates/dash_h', $data);
        // Pastikan Anda membuat file view baru dengan nama ini
