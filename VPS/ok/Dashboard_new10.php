...
class Dashboard_new extends MY_Controller
{
    ...
    public function visual_kasus_penyakit()
    {
        ...
        $pivot_chart_data = []; // Array untuk memivot data: $pivot_chart_data[kategori][bulan_tahun] = persentase
        ...
        foreach ($raw_stacked_data as $row) {
            ...
            // Simpan data persentase ke array pivot
            $pivot_chart_data[$kat][$bulan_tahun] = $persentase;
        }
        ...
        // --- [AKHIR PERUBAHAN UNTUK MENAMPILKAN JANUARI-DESEMBER] ---
    ...
    public function visual_kandang_kosong()
    {
        ...
        $data['chart_datasets'] = json_encode($datasets); 
        // --- [AKHIR PROCESSING CHART] ---
        

        // --- [AWAL TAMBAHAN DATA TABEL] ---
        // Panggil model baru, teruskan filter tipe ternak yang sama dengan chart
        $data['farm_capacity_list'] = $this->visual->get_farm_capacity_list($selected_tipe_ternak);
        // --- [AKHIR TAMBAHAN DATA TABEL] ---

        // Kirim data filter kembali ke view
...
public function visual_kondisi_lingkungan()
{
    ...
    // // *** FUNGSI INI DIPERBAIKI: 'fill' => false DAN DITAMBAH FILTER TAHUN ***
    ...
    //             // Pastikan perbandingan sebagai string jika $selected_years dari POST
    ...
    public function visual_harga_compare()
    {
        ...
        // 1. Ambil semua tahun dari DB (pastikan fungsi ini ada di M_Visual)
        $all_years_raw = $this->visual->get_all_harga_years();
        $data['all_years'] = $all_years_raw ? array_column($all_years_raw, 'tahun') : [date('Y')];
        ...
        // 1. Ambil Data Stat Cards (Harga Terakhir) - (Tidak berubah)
        $data['stat_jagung'] = $this->visual->get_harga_terbaru_by_jenis('harga_jagung');
        $data['stat_katul'] = $this->visual->get_harga_terbaru_by_jenis('harga_katul');
        ...
        // 2. Ambil Data Bulanan (Semua Tahun) - (Tidak berubah)
        $raw_telur = $this->_get_monthly_data_by_commodity('telur', null);
        $raw_lb = $this->_get_monthly_data_by_commodity('live_bird', null);
        $raw_jagung = $this->_get_monthly_data_by_commodity('jagung', null);
        $raw_katul = $this->_get_monthly_data_by_commodity('katul', null);
        ...
        // 3. Proses Data untuk Chart
        // --- [PERUBAHAN] Kirim $selected_years ke semua helper ---
        $data['chart_hpp_konsentrat_vs_telur'] = $this->_process_comparison_chart_data(
            $raw_hpp_konsentrat, "HPP (Konsentrat)",
            $raw_telur, "Harga Telur",
            $selected_years // <-- Parameter baru
        );
    ...
    /**
     * [CRM] Halaman utama untuk visualisasi data CRM Broiler.
     */
    public function visual_data_crm()
    {
        ...
        // Filter hak akses
        // 'master_area_id' di session diasumsikan map ke 'region_id'
        if (isset($data['user']['group_user'])) {
            $group = $data['user']['group_user'];
            if ($group === 'surveyor') {
                $user_id_filter = $data['user']['id_user'];
        ...
        // --- [LOGIKA FILTER TANGGAL (SAMA SEPERTI LAMA)] ---
    ...
    /**
     * [CRM] AJAX: Mengambil data komposisi untuk 1 Surveyor.
     * Disederhanakan: tidak lagi mengambil area_data, karena JS tidak menggunakannya.
     */
    public function get_crm_data_for_surveyor_ajax()
    {
        ...
        // 1. Dapatkan data Komposisi Visit untuk user tersebut
        $area_id_filter = null;
        $visit_breakdown_raw = $this->visual->get_crm_visit_breakdown($selected_start, $selected_end, $user_id, $area_id_filter);

        // 2. Logika kalkulasi persentase (disederhanakan, tanpa mapping)
        $combined_data = $visit_breakdown_raw;
        $grand_total = array_sum(array_column($combined_data, 'jumlah_visit'));
        $final_breakdown = [];
        if ($grand_total > 0) {
        ...
        $response = [
            'status' => 'success',
            // 'area_data' dihilangkan karena JS tidak menggunakannya
            'composition_data' => $composition_data,
        ];
    ...
    // {
    //     // 1. Ambil input area dan tahun dari POST request AJAX
    //     $area_id = $this->input->post('area_id');
    //     $selected_year = $this->input->post('tahun');
    ...
    //     // Tentukan filter untuk Model: Jika area_id > 0, kita pakai filter area. Jika 0, tidak ada filter area.
    //     $area_filter_to_model = (int)$area_id > 0 ? $area_id : null;
    //     $user_id_filter = null; // Asumsi di halaman ini, filter perorangan diabaikan saat filter per Area.

    //     // 2. Ambil data yang dibutuhkan (sama seperti di visual_kasus_penyakit)
    //     // NOTE: Anda perlu membuat M_Visual.php agar menerima area_id juga di semua fungsi ini.

    //     // A. Data Grafik Kasus per Bulan
    //     $raw_stacked_data = $this->visual->get_kasus_breakdown_stacked($selected_year, $user_id_filter, $area_filter_to_model);
        
    //     // Logika Processing Grafik (SAMA PERSIS DENGAN visual_kasus_penyakit())
    //     $master_labels_bulan = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
    //     $kategori_chart = [];
    //     $pivot_chart_data = [];
    //     foreach ($raw_stacked_data as $row) {
    ...
    //     }
    //     $datasets = [];
    //     $colors = ['#dc3545', '#7b23b7ff', '#ffc107', '#28a745', '#fd7e14', '#17a2b8', '#6c757d'];
    //     $color_index = 0;
    //     foreach ($kategori_chart as $kat) {
    //         $dataset = ['label' => $kat, 'data' => [], 'backgroundColor' => $colors[$color_index % count($colors)]];
    //         foreach ($master_labels_bulan as $bulan) {
    //             $bulan_query = date('M Y', mktime(0, 0, 0, array_search($bulan, $master_labels_bulan) + 1, 1, $selected_year));
    //             $dataset['data'][] = $pivot_chart_data[$kat][$bulan_query] ?? 0;
    //         }
    //         $datasets[] = $dataset;
    //         $color_index++;
    ...
    //     $chart_data_response = [
    //         'labels' => $master_labels_bulan,
    //         'datasets' => $datasets
    //     ];

    //     // B. Data Rincian Laporan Kasus
    ...
    //         'new_csrf_hash' => $this->security->get_csrf_hash() // Penting untuk keamanan
    //     ];

    //     $this->output->set_content_type('application/json')->set_output(json_encode($response));
    // }

    // FUNGSI BARU UNTUK AJAX: Dashboard_new.php
public function get_kasus_data_for_area_ajax()
{
    // 1. Ambil input area dan tahun dari POST request AJAX
    $area_id = $this->input->post('area_id');
    ...
    // Tentukan filter untuk Model
    $area_filter_to_model = (int)$area_id > 0 ? $area_id : null;
    $user_id_filter = null; 

    // 2. Ambil data yang dibutuhkan (dengan filter area)
    $raw_stacked_data = $this->visual->get_kasus_breakdown_stacked($selected_year, $user_id_filter, $area_filter_to_model);
    $detail_list = $this->visual->get_kasus_detail_list($selected_year, $user_id_filter, $area_filter_to_model);
    
    // Logika Processing Grafik (Membuat format datasets untuk Chart.js)
    $master_labels_bulan = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
    $kategori_chart = []; $pivot_chart_data = [];
    foreach ($raw_stacked_data as $row) {
        $bulan_tahun = $row['bulan_tahun'];
        $kat = $row['kategori_kasus'];
        $persentase = (float)$row['persentase'];
    ...
    foreach ($kategori_chart as $kat) {
        $dataset = ['label' => $kat, 'data' => [], 'backgroundColor' => $colors[$color_index % count($colors)]];
        for ($i = 1; $i <= 12; $i++) {
            $bulan = $master_labels_bulan[$i - 1];
            $bulan_query = date('M Y', mktime(0, 0, 0, $i, 1, $selected_year));
            $dataset['data'][] = $pivot_chart_data[$kat][$bulan_query] ?? 0;
        }
        $datasets[] = $dataset;
        $color_index++;
    ...
    $response = [
        ...
        'chart_data' => $chart_data_response,
        'detail_list' => $detail_list,
        'new_csrf_hash' => $this->security->get_csrf_hash() // Penting: Mengupdate CSRF token
    ];
