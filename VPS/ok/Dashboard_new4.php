...
class Dashboard_new extends MY_Controller
{
    ...
    private function _get_monthly_data_by_commodity($jenis_komoditas, $tahun = null)
    {
        ...
        switch($jenis_komoditas) {
            ...
            case 'pakan_campuran':
                return $this->visual->get_harga_pakan_campuran_bulanan_chart($tahun);
            case 'pakan_komplit_layer':
                return $this->visual->get_harga_pakan_komplit_layer_bulanan_chart($tahun);
...
public function visual_harga_gabungan()
    ...
    {
        $data['user'] = $this->dash->getUserInfo($this->session->userdata('token'))->row_array();        $data["title"] = "Laporan Harga Komoditas Utama";
    ...
    // GANTI FUNGSI LAMA _process_monthly_chart_data() DENGAN YANG INI
    private function _process_monthly_chart_data($raw_data)
    {
        ...
        foreach ($years_found as $tahun) {
            ...
            $datasets[] = [
                ...
                'data' => $dataset_data,
                'borderColor' => $color,
                'backgroundColor' => $color . '20', // Opacity 12.5%
                'fill' => true,
                'tension' => 0.4,
                'borderWidth' => 3,
                'pointRadius' => 4,
