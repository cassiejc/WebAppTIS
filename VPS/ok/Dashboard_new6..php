...
class Dashboard_new extends MY_Controller
{
    ...
    public function visual_kondisi_lingkungan()
    {
        ...
        $master_labels = [];
        for ($m = 1; $m <= 12; $m++) {
            $master_labels[] = date('M Y', mktime(0, 0, 0, $m, 1, $selected_year));
        }
        ...
        $process_100_percent_stacked_data = function($raw_data, $chart_key, $master_labels) {
            ...
            foreach ($raw_data as $row) {
                ...
                $bulan = $row['bulan_tahun']; $kat = $row['nilai']; $jumlah = (int)$row['jumlah'];
        ...
        // 7. [PERUBAHAN] Proses Data AVERAGE (Suhu, Kelembapan, HI)
        $process_multi_axis_chart = function($raw_data, $master_labels) {
            $pivot_data = [];
            // Pivot data: [ 'Jan 2025' => ['suhu' => 29.5, 'kelembapan' => 80, 'heat_index' => 165.1], ... ]
            foreach($raw_data as $row) {
            ...
            foreach ($master_labels as $bulan) {
                ...
                $heat_index_values[] = $pivot_data[$bulan]['heat_index'] ?? null; // <-- BARU
            }
            ...
            return [
                ...
                'datasets' => [
                    [
                        ...
                        'label' => 'Suhu (°C)',
                        'data' => $suhu_values,
                        'backgroundColor' => 'rgba(220, 53, 69, 0.7)', // Merah transparan
                        'borderColor' => '#dc3545',
                        'yAxisID' => 'ySuhu', // Sumbu Y kiri
                        'order' => 2 // Render bar di belakang
                    ],
                    [
                        'type' => 'bar', // Tipe Bar
                        'label' => 'Kelembapan (%)',
                        ...
                        'borderColor' => '#007bff',
                        'yAxisID' => 'yKelembapan', // Sumbu Y kanan (utama)
                        'order' => 2 // Render bar di belakang
                    ],
                    [ // <-- DATASET BARU
                        'type' => 'line', // Tipe Line
                        ...
                        'yAxisID' => 'yHeatIndex', // Sumbu Y kanan (sekunder)
                        'tension' => 0.4,
                        'borderWidth' => 3,
                        'pointRadius' => 4,
                        'order' => 1 // Render line di depan
                    ]
                ]
            ];
        };
    ...
    public function visual_harga_compare()
    {
        ...
        $data['stat_pakan_layer'] = $this->visual->get_harga_terbaru_by_jenis('hpp_komplit_layer'); // Sesuai mapping Anda
