...
class M_Visual extends CI_Model {
    ...
    // Kategori yang HANYA ingin ditampilkan sesuai permintaan user
    $allowed_categories = "'Bacterial', 'Parasit', 'Jamur', 'Viral'"; // <-- Filter baru
    ...
    // Subquery untuk memfilter kategori yang diinginkan
    $filtered_kasus_query = "
        SELECT 
            bulan_tahun, 
            kategori_kasus
        FROM ({$union_query}) as semua_kasus_raw
        WHERE kategori_kasus IN ({$allowed_categories})
    ";
    ...
    $base_count_query = "
        SELECT 
            ...
            kategori_kasus, 
            COUNT(*) as jumlah
        FROM ({$filtered_kasus_query}) as semua_kasus_filtered
    ...
    public function get_kasus_detail_list($year, $user_id = null) {
    // ... (Filter Year & User sama seperti sebelumnya) ...
    // ... (HANYA MENGGANTI BAGIAN SELECT DAN JOIN) ...
    $peternakan_tables = [
        'visiting_p_grower', 'visiting_p_bebek_pedaging', 'visiting_p_layer',
        'visiting_p_bebek_petelur', 'visiting_p_puyuh', 'visiting_p_arap', 'visiting_p_lainnya'
    ];
    ...
    $final_query = "
        ...
        LEFT JOIN master_area ma ON u.master_area_id = ma.master_area_id -- JOIN untuk ambil nama area
        ORDER BY waktu_kunjungan DESC
    ";
...
public function get_lingkungan_avg_monthly($year, $user_id = null, $area_id = null, $pakan_filter = [], $selected_areas = []) { 
    ...
    /**
     * [BARU] Mengambil daftar kapasitas farm saat ini (end_date = 9999)
     * Disesuaikan dengan filter Tipe Ternak
     */
    public function get_farm_capacity_list($tipe_ternak = null)
    {
        $this->db->select('mf.nama_farm, mf.kapasitas_farm, hfc.start_date');
        $this->db->from('master_farm mf');
        // Gunakan LEFT JOIN untuk keamanan jika ada farm tanpa history
        $this->db->join('history_farm_capacity hfc', 'mf.master_farm_id = hfc.master_farm_id', 'left');
        
        // Kondisi utama sesuai permintaan Anda
        $this->db->where('hfc.end_date', '9999-12-31');

        // Terapkan filter Tipe Ternak jika ada (agar konsisten dengan chart)
        if (!empty($tipe_ternak)) {
            $this->db->where('mf.tipe_ternak', $tipe_ternak);
        } else {
            // Jika "Semua", tetap kecualikan "Lainnya" agar konsisten dengan dropdown
    ...
    /**
     * [CRM] Mengambil performa surveyor (hanya total aktual).
     */
