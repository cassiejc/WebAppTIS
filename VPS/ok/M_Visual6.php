...
class M_Visual extends CI_Model {
    ...
    //             DATE_FORMAT(t.waktu_kunjungan, '%b') as bulan_tahun, -- <-- UBAH
    //             MONTH(t.waktu_kunjungan) as bulan_num, -- <-- TAMBAH
    ...
    //         AND TRIM(t.kondisi_lalat_layer) != '-'
    //         GROUP BY bulan_tahun, bulan_num, nilai -- <-- UBAH
    //     ";
    ...
    //     // Query Suhu dan Kelembapan telah dihapus dari sini

    //     $final_query = "{$query_lalat} 
    //                         UNION ALL 
    //                         {$query_kotoran}
    //                         ORDER BY bulan_num, kategori_chart, nilai"; // <-- UBAH
        
    //     return $this->db->query($final_query)->result_array();
    // }

    // GANTI FUNGSI INI DI M_Visual.php
// public function get_kondisi_lingkungan_monthly($year, $user_id = null, $area_id = null, $pakan_filter = [], $selected_areas = []) { // <-- 1. TAMBAH $selected_areas
//     $year = (int)$year;
//     $base_filters = ''; 
...
//     // Filter user/area (HAK AKSES)
//     if ($user_id !== null) {
//         $base_filters .= " AND t.id_user = {$user_id}";
//     }
    
//     // [PERUBAHAN LOGIKA]
//     // Filter Koordinator ATAU Filter Area dari Form (jika Admin)
//     elseif ($area_id !== null) { 
//         // Filter Koordinator
//         $base_filters .= " AND u.master_area_id = {$area_id}";
//     } 
//     elseif (empty($user_id) && empty($area_id) && !empty($selected_areas)) {
//         // 2. TAMBAH BLOK INI: Filter Form untuk Admin
//         $area_in_clause = implode(',', array_map('intval', $selected_areas));
//         if (!empty($area_in_clause)) {
//             $base_filters .= " AND u.master_area_id IN ({$area_in_clause})";
//         }
//     }
...
//     // ... (Sisa query_lalat dan query_kotoran tetap sama) ...
...
//         AND TRIM(t.kondisi_lalat_layer) != '' 
//         AND TRIM(t.kondisi_lalat_layer) != '-'
//         GROUP BY bulan_tahun, bulan_num, nilai 
//     ";
...
//                     UNION ALL 
//                     {$query_kotoran}
//                     ORDER BY bulan_num, kategori_chart, nilai"; 
    
//     return $this->db->query($final_query)->result_array();
// }

// GANTI FUNGSI INI DI M_Visual.php
public function get_kondisi_lingkungan_monthly($year, $user_id = null, $area_id = null, $pakan_filter = [], $selected_areas = []) { 
    $year = (int)$year;
    $base_filters = ''; 
    ...
    // Filter user/area (HAK AKSES - TETAP PAKAI DATA USER)
    if ($user_id !== null) {
        // Filter Surveyor
        $base_filters .= " AND t.id_user = {$user_id}";
    }
    ...
    // [PERUBAHAN LOGIKA]
    // Filter Area dari Form (jika Admin) - SEKARANG PAKAI DATA FARM
    elseif (empty($user_id) && empty($area_id) && !empty($selected_areas)) {
        $area_in_clause = implode(',', array_map('intval', $selected_areas));
        if (!empty($area_in_clause)) {
            // Filter berdasarkan area FARM-nya, bukan area USER-nya
            $base_filters .= " AND mf.master_area_id IN ({$area_in_clause})"; // <-- [UBAH]
        }
    }
    // [AKHIR PERUBAHAN LOGIKA]
    ...
    $query_lalat = "
        ...
        FROM visiting_p_layer t
        LEFT JOIN z_master_user u ON t.id_user = u.id_user
        LEFT JOIN master_farm mf ON t.layer_nama_farm = mf.nama_farm -- <-- [BARU] JOIN ke master_farm
        {$base_filters}
        AND TRIM(t.kondisi_lalat_layer) IS NOT NULL 
        AND TRIM(t.kondisi_lalat_layer) != '' 
        AND TRIM(t.kondisi_lalat_layer) != '-'
    ...
    // public function get_lingkungan_avg_monthly($year, $user_id = null, $area_id = null, $pakan_filter = []) {
    //     $year = (int)$year;
    //     $base_filters = ''; 
    ...
    //     // Filter REGEXP untuk memastikan hanya data numerik yang di-cast
    ...
    //         GROUP BY bulan_tahun, bulan_num -- <-- UBAH
    //     ";
    ...
    //     // --- [BARU] Query untuk Heat Index ---
    //     // Rumus: ( (AvgSuhuC * 9/5) + 32 ) + AvgKelembapan
    //     $query_heat_index = "
    //         SELECT
    ...
    //                         UNION ALL
    //                         {$query_heat_index}
    //                         ORDER BY bulan_num, kategori_chart"; // <-- UBAH
    ...
    //     // [PERUBAHAN] Kita gunakan array untuk membangun klausa WHERE dengan aman
    //     $where_clauses = []; 

    //     // 1. Filter Tahun (Tetap digunakan)
    //     if ($year != 0) {
    //         $where_clauses[] = "YEAR(t.waktu_kunjungan) = {$year}";
    //     }
        
    //     // 2. [DIHAPUS] Filter 'Full CP' dihilangkan dari fungsi ini
    //     // $full_cp_filter = "TRIM(t.layer_pilihan_pakan_lain) = 'Full CP'";
    //     // $where_clauses[] = $full_cp_filter;
        
    //     // 3. Filter user/area (Tetap digunakan untuk hak akses)
    //     if ($user_id !== null) {
    //         $where_clauses[] = "t.id_user = {$user_id}";
    //     }
        
    //     if ($area_id !== null && $user_id === null) { 
    //         $where_clauses[] = "u.master_area_id = {$area_id}";
    //     }
        
    //     // 4. [DIHAPUS] Filter Pakan (Checkbox) dihilangkan dari fungsi ini
    //     // if (!empty($pakan_filter) && is_array($pakan_filter)) {
    //     //     $escaped_pakan = [];
    //     //     foreach ($pakan_filter as $pakan) {
    //     //         if(!empty(trim($pakan))) {
    ...
    //     //     if (!empty($escaped_pakan)) {
    //     //         $pakan_in_clause = implode(',', $escaped_pakan);
    //     //         $pakan_filter_sql = "(TRIM(t.layer_pilihan_pakan_cp) IN ({$pakan_in_clause}))";
    //     //         $where_clauses[] = $pakan_filter_sql;
    //     //     }
    //     // }

    //     // Gabungkan semua klausa yang tersisa
    //     $base_filters = "";
    ...
    // GANTI FUNGSI INI DI M_Visual.php
// public function get_lingkungan_avg_monthly($year, $user_id = null, $area_id = null, $pakan_filter = [], $selected_areas = []) { // <-- 1. TAMBAH $selected_areas
//     $year = (int)$year;
//     $where_clauses = []; 
...
//         $area_in_clause = implode(',', array_map('intval', $selected_areas));
//         if (!empty($area_in_clause)) {
//             $where_clauses[] = "u.master_area_id IN ({$area_in_clause})";
//         }
//     }
//     // [AKHIR PERUBAHAN LOGIKA]

//     // [DIHAPUS] Filter pakan dan Full CP memang sudah dihapus dari sini, biarkan saja.
...
//     // ... (Sisa query_suhu, query_kelembapan, query_heat_index tetap sama) ...
...
// GANTI FUNGSI INI DI M_Visual.php
public function get_lingkungan_avg_monthly($year, $user_id = null, $area_id = null, $pakan_filter = [], $selected_areas = []) { 
    $year = (int)$year;
    $where_clauses = []; 
    ...
    elseif (empty($user_id) && empty($area_id) && !empty($selected_areas)) {
        ...
        if (!empty($area_in_clause)) {
            // Filter berdasarkan area FARM-nya, bukan area USER-nya
            $where_clauses[] = "mf.master_area_id IN ({$area_in_clause})"; // <-- [UBAH]
        }
    }
    // [AKHIR PERUBAHAN LOGIKA]
    ...
    $query_suhu = "
        ...
        FROM visiting_p_layer t
        LEFT JOIN z_master_user u ON t.id_user = u.id_user
        LEFT JOIN master_farm mf ON t.layer_nama_farm = mf.nama_farm -- <-- [BARU] JOIN
        {$base_filters} {$numeric_check_suhu}
        GROUP BY bulan_tahun, bulan_num 
    ";
