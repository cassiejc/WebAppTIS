...
class Dashboard_new extends MY_Controller
{
    ...
    // GANTI FUNGSI LAMA _get_monthly_data_by_commodity() DENGAN YANG INI
    private function _get_monthly_data_by_commodity($jenis_komoditas, $tahun)
    {
        // $tahun sekarang DITERIMA sebagai parameter
        // $tahun = null; // <- Hapus baris ini
        ...
        switch($jenis_komoditas) {
            ...
            case 'katul':
                // Model ini tidak menerima $tahun, biarkan saja
...
public function visual_harga_gabungan()
    ...
    // Tambahkan method ini di dalam class Dashboard_new
    // GANTI FUNGSI INI DI Dashboard_new.php (mulai sekitar baris 411)
    ...
    // GANTI FUNGSI LAMA visual_harga() DENGAN YANG INI
    public function visual_harga($jenis_komoditas_usang = 'telur')
    {
        $data['user'] = $this->dash->getUserInfo($this->session->userdata('token'))->row_array();
        $data["title"] = "Laporan Harga Komoditas Utama";
        
        // Tentukan Tahun yang akan ditampilkan (Tahun ini)
        $selected_year = date('Y');
        $data['display_year'] = $selected_year; // Kirim ke view untuk judul

        // 1. Ambil DATA HARGA TERAKHIR (Stat Cards) - (Ini tidak berubah)
        ...
        // Kita teruskan $selected_year ke helper
        $raw_telur = $this->_get_monthly_data_by_commodity('telur', $selected_year);
        $raw_puyuh = $this->_get_monthly_data_by_commodity('telur_puyuh', $selected_year);
        $raw_bebek = $this->_get_monthly_data_by_commodity('telur_bebek', $selected_year);
        $raw_lb    = $this->_get_monthly_data_by_commodity('live_bird', $selected_year);
        $raw_afkir = $this->_get_monthly_data_by_commodity('afkir', $selected_year);

        // 3. Proses data untuk dikirim ke Javascript
        // Kita teruskan $selected_year ke helper
        $data['chart_telur'] = $this->_process_monthly_chart_data($raw_telur, $selected_year);
        $data['chart_puyuh'] = $this->_process_monthly_chart_data($raw_puyuh, $selected_year);
        $data['chart_bebek'] = $this->_process_monthly_chart_data($raw_bebek, $selected_year);
        $data['chart_lb']    = $this->_process_monthly_chart_data($raw_lb, $selected_year);
        $data['chart_afkir'] = $this->_process_monthly_chart_data($raw_afkir, $selected_year);
    ...
    // GANTI FUNGSI _process_monthly_chart_data LAMA DENGAN YANG INI
    private function _process_monthly_chart_data($raw_data, $tahun)
    {
        $labels_final = []; 
        $data_final = [];
        $nama_bulan = [
        ...
        // 1. Buat "Lookup" dari data mentah
        $lookup_data = [];
        foreach ($raw_data as $row) {
            if ($row['tahun'] == $tahun) {
                $bulan_int = (int)$row['bulan'];
                $lookup_data[$bulan_int] = (float)$row['nilai_rata_rata'];
            }
        }
        
        // 2. Bangun array final dengan 12 bulan (Jan - Des)
        for ($m = 1; $m <= 12; $m++) {
            // Label (Contoh: "Jan 2024")
            $labels_final[] = $nama_bulan[$m] . ' ' . $tahun;
            
            // Data: Ambil dari $lookup_data jika ada, jika tidak, isi null
            // Menggunakan 'null' akan membuat garis terputus di Chart.js
            $data_final[] = $lookup_data[$m] ?? null; 
        }
        
        // 3. Kembalikan data yang sudah lengkap 12 bulan
        return [
            'labels' => json_encode($labels_final), 
            'data' => json_encode($data_final)
        ];
    }

    // BARU: Fungsi untuk Perbandingan Harga
    public function visual_harga_compare()
    {
        ...
        $data['selected_komoditas1'] = $this->input->get('komoditas1') ?: 'telur';
        $data['selected_komoditas2'] = $this->input->get('komoditas2') ?: 'jagung';
        $data['selected_tahun'] = $this->input->get('tahun') ?: 'semua'; // Nama filter di view Anda adalah 'tahun'
        ...
        // Ambil data bulanan untuk 2 komoditas
        $data1_raw = $this->_get_monthly_data_by_commodity($data['selected_komoditas1']);
        $data2_raw = $this->_get_monthly_data_by_commodity($data['selected_komoditas2']);
        ...
        foreach ($data1_raw as $row) {
            ...
            if ($data['selected_tahun'] == 'semua' || $row['tahun'] == $data['selected_tahun']) {
                $all_labels_sortable[$key] = $label;
                $pivot_data[$key][$data['selected_komoditas1']] = (float)$row['nilai_rata_rata'];
            }
        }
        ...
        ksort($all_labels_sortable); // Urutkan berdasarkan $key (Tahun-Bulan)
        ...
        // Load View
        $this->load->view('templates/dash_h', $data);
        $this->load->view('visual_harga_compare_view', $data); // <-- Memuat view baru Anda
        $this->load->view('templates/dash_f', $data);
    }
} 
