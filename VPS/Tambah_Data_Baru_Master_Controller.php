...
class Tambah_Data_Baru_Master_Controller extends CI_Controller {
    ...
    private function _handle_form($kategori, $page) {
        ...
        // if ($kategori == 'Farm' && array_key_exists('role', $user) && !in_array($user['role'], ['Admin', 'Koordinator'])) {            $data['questions_kategori'] = array_filter($data['questions_kategori'], function($q) {
        //         return $q['field_name'] !== 'master_sub_area_id';
        //     });
        // }
        // if ($kategori == 'Farm' && array_key_exists('role', $user) && !in_array($user['role'], ['Admin', 'Koordinator'])) {
        //     $filtered_questions = [];
        //     foreach ($data['questions_kategori'] as $question) {
        //         // Jika field_name BUKAN 'master_sub_area_id', maka simpan pertanyaan ini
        //         if ($question['field_name'] !== 'master_sub_area_id') {
        //             $filtered_questions[] = $question;
        //         }
        //     }
        //     // Ganti pertanyaan original dengan yang sudah difilter
        //     $data['questions_kategori'] = $filtered_questions;
        // }

        // ... di dalam _handle_form() ...

        // Cek jika form adalah 'Farm'
        // ... kode sebelum ini
        ...
        // Cek jika form adalah 'Farm'
        // HAPUS BLOK IF LAMA ANDA, DAN GANTI DENGAN YANG INI
        // ==========================================================

        // Logika untuk menampilkan/menyembunyikan field area berdasarkan peran
        if ($kategori == 'Farm' && array_key_exists('group_user', $user) && !is_null($user['group_user'])) {
            $user_group = strtolower($user['group_user']);

            // Tentukan field mana yang akan dihapus dari form
            $fields_to_remove = [];

            if ($user_group !== 'administrator') {
                // Jika BUKAN Admin (Koordinator, Surveyor, dll), sembunyikan pilihan Area
                $fields_to_remove[] = 'master_area_id';
            }

            if ($user_group !== 'administrator' && $user_group !== 'koordinator') {
                // Jika BUKAN Admin DAN BUKAN Koordinator (misal: Surveyor),
                // sembunyikan juga pilihan Sub Area
                $fields_to_remove[] = 'master_sub_area_id';
            }

            // Jika ada field yang perlu dihapus, lakukan proses filter
            if (!empty($fields_to_remove)) {
                $filtered_questions = [];
                foreach ($data['questions_kategori'] as $question) {
                    // Hanya simpan pertanyaan yang field_name-nya TIDAK ADA di daftar hapus
                    if (!in_array($question['field_name'], $fields_to_remove)) {
                        $filtered_questions[] = $question;
                    }
                }
                // Ganti daftar pertanyaan dengan yang sudah difilter
                $data['questions_kategori'] = $filtered_questions;
            }
        }
        // ==========================================================
...
// ... sisa kode
    ...
    private function _process_form_submission($questions_kategori, $kategori, $user, $page) {
        ...
        // Logika kondisional untuk Farm
        $user_group = (array_key_exists('group_user', $user) && !is_null($user['group_user'])) ? strtolower($user['group_user']) : '';

        if ($user_group !== 'administrator') {
            // Jika bukan admin (Koordinator/Surveyor), ambil area dari sesi
            $save_data['master_area_id'] = $user['master_area_id'];
        }
        // Jika admin, area_id akan diambil dari form (oleh loop foreach di bawah)

        if ($user_group === 'surveyor' || $user_group === '') { // atau peran lain yang tidak memilih
             // Jika surveyor (atau peran lain), ambil sub_area dari sesi
            $save_data['master_sub_area_id'] = $user['master_sub_area_id'];
        }
    
        } elseif ($kategori == 'Sub Agen') { // Sub Agen butuh keduanya dari user
            $save_data['master_sub_area_id'] = $user['master_sub_area_id'];
            $save_data['master_area_id'] = $user['master_area_id'];
    ...
    private function _save_data($kategori, $save_data, $user, $page) {
        ...
        switch($kategori) {
            ...
            if ($insert_success) {
                ...
                // INI BARIS YANG BENAR (5 argumen)
                // Ambil area dan sub_area dari $save_data agar sesuai dengan pilihan admin/koordinator
                $sub_area_id = $save_data['master_sub_area_id'] ?? null;
                $area_id = $save_data['master_area_id'] ?? null;
                
                $this->_add_farm_to_options($save_data, $sub_area_id, $area_id, $new_farm_id, $user['id_user']);
            }
            break;
    ...
    private function _add_farm_to_options($save_data, $master_sub_area_id, $master_area_id, $master_farm_id, $id_user) {
