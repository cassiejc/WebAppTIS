...
public function visual_harga_gabungan()
    ...
    public function get_data_for_surveyor_ajax()
    {
        ...
        if (empty($user_id)) {
            $this->output->set_status_header(400)->set_output(json_encode(['error' => 'User ID tidak valid.']));
            return;
        }
        ...
        // Proses ini disalin dari visual_data_kunjungan, tapi difilter untuk satu $user_id
        ...
        // Lakukan grouping dan kalkulasi persentase (sama seperti di method visual_data_kunjungan)
        $data_to_group = $visit_breakdown_raw;
        if ($sample_count > 0) $data_to_group[] = ['kategori' => 'Sample', 'jumlah_visit' => $sample_count];
        ...
        $response = [
            ...
            // Kita tidak perlu mengirim 'log_data' karena kita akan memfilternya di client-side
        ];
    ...
    //     if (empty($area_id)) {
    //         $this->output->set_status_header(400)->set_output(json_encode(['error' => 'Area ID tidak valid.']));
    //         return;
    //     }

    //     // Panggil fungsi performa surveyor, tapi kali ini dengan filter $area_id
    //     // Parameter ke-3 (user_id) adalah null, parameter ke-4 (area_id) diisi
    ...
    public function get_surveyors_for_area_ajax()
    {
        ...
        // 1. Panggil fungsi performa surveyor (Sudah ada)
        $surveyor_data = $this->visual->get_surveyor_performance(
            $selected_month, 
            $selected_year, 
            null, 
        ...
        $visit_breakdown_raw = $this->visual->get_visit_breakdown($selected_month, $selected_year, $user_id_filter, $area_id);
        $sample_count = $this->visual->get_sample_count_by_month($selected_month, $selected_year, $user_id_filter, $area_id);
        $seminar_count = $this->visual->get_seminar_count_by_month($selected_month, $selected_year, $user_id_filter, $area_id);
        $new_customer_count = $this->visual->get_new_customer_count_by_month($selected_month, $selected_year, $user_id_filter, $area_id);
