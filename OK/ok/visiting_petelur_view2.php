...
.farm-capacity-info {
    color: #0d6efd;
    font-weight: bold;
    /* Hilangkan margin-left yang berlebihan */
    margin-left: 0; 
...
#refreshKapasitasBtn {
    ...
    font-size: 0.8rem;
    /* Sesuaikan tinggi tombol agar pas */
    margin-top: -3px; 
}
/* Hapus atau modifikasi .farm-capacity-group jika Anda menggunakannya di langkah sebelumnya. */

/* BARU: Container untuk Kapasitas dan Tombol */
.capacity-and-refresh-container {
    /* Sesuaikan dengan lebar maksimal input dan label */
    max-width: 400px; 
    display: flex;
    align-items: center;
    justify-content: space-between; /* Membuat tulisan kapasitas di kiri dan tombol di kanan */
}
    </style>
</head>
<body>
    ...
    // --- FUNGSI DROPDOWN (DIPERBARUI UNTUK KAPASITAS & VIP) ---
    function selectOption(qId, text, element) {
        ...
        // **MODIFIKASI: Simpan kapasitas DAN status VIP**
        ...
        // 2. BARU: Handle VIP Status
        // Ambil data-vip_farm. Ini akan berisi "Ya" atau "Tidak"
        const vip_status = element.dataset.vip_farm; 
        const vipHiddenInput = document.getElementById('selected_vip_farm');
        if (vipHiddenInput) {
            // Langsung set nilainya, karena sudah "Ya" atau "Tidak"
    ...
    /**
     * Helper untuk mereset pilihan farm dan kapasitas (DAN VIP)
     */
    function resetSelectedFarm() {
        ...
        // BARU: Reset hidden input VIP farm ke default "Tidak"
        const vipHiddenInput = document.getElementById('selected_vip_farm');
        if (vipHiddenInput) {
        ...
        /**
     * Fungsi utama untuk refresh data farm via AJAX (KOREKSI: Mempertahankan pilihan farm)
     */
    function refreshFarmData(button) {
        ...
        const hiddenInput = document.getElementById(`dd_hidden_${questionId}`);
        const buttonElement = document.getElementById(`dd_btn_${questionId}`);

        // 1. SIMPAN NILAI FARM YANG SEDANG DIPILIH
        const currentlySelectedFarm = hiddenInput ? hiddenInput.value : '';
        let foundNewCapacity = currentSelectedKapasitas;
        let foundNewVipStatus = document.getElementById('selected_vip_farm').value; 
            ...
            if (xhr.readyState === 4) {
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        // Bangun ulang opsi-opsi & cari yang dipilih
                        if (newOptions && newOptions.length > 0) {
                            newOptions.forEach(o => {
                                ...
                                // 2. JIKA DITEMUKAN, SIMPAN KAPASITAS & VIP YANG BARU
                                if (o.option_text === currentlySelectedFarm) {
                                    foundNewCapacity = parseInt(kapasitas, 10);
                                    foundNewVipStatus = vip_status;
                                }
                            });
                        } else {
                        ...
                        // 3. SET KEMBALI PILIHAN FARM DAN KAPASITAS BARU
                        if (currentlySelectedFarm && buttonElement && hiddenInput) {
                            hiddenInput.value = currentlySelectedFarm;
                            buttonElement.querySelector('span').textContent = currentlySelectedFarm;
                            
                            // Update Kapasitas Global dan Tampilan
                            currentSelectedKapasitas = foundNewCapacity;
                            document.getElementById('selected_vip_farm').value = foundNewVipStatus;

                            const capacityDiv = document.getElementById('kapasitas_caption_div');
                            if (capacityDiv) {
                                const formatted = foundNewCapacity.toLocaleString('en-US');
                                capacityDiv.textContent = `Kapasitas farm: ${formatted}`;
                            }
                            
                            Swal.fire('Sukses', `Data farm diperbarui. Kapasitas: ${foundNewCapacity.toLocaleString('en-US')}`, 'success');

                        } else {
                            // Jika tidak ada yang dipilih atau pilihan lama tidak ditemukan
                            Swal.fire('Sukses', 'Data farm berhasil diperbarui. Silakan pilih farm.', 'success');
                            // Lakukan reset penuh (ini sama dengan logika lama di resetSelectedFarm)
                            if (hiddenInput) hiddenInput.value = '';
                            if (buttonElement) buttonElement.querySelector('span').textContent = '-- Pilih --';
                            const capacityDiv = document.getElementById('kapasitas_caption_div');
                            if (capacityDiv) capacityDiv.textContent = 'Kapasitas farm: -';
                            currentSelectedKapasitas = 0;
                            document.getElementById('selected_vip_farm').value = 'Tidak';
                        }
    ...
    function changeTipeTermak(tipeTermak) {
        ...
        // BARU: Reset juga VIP farm saat ganti tipe ternak
        const vipHiddenInput = document.getElementById('selected_vip_farm');
        if (vipHiddenInput) {
            vipHiddenInput.value = 'Tidak';
        }
        ...
        if (q.type === 'select' && q.options) {
            ...
            if (isSearchable) {
                ...
                const optionsHTML = q.options.map(opt => {
                    // **MODIFIKASI: Tambahkan data-kapasitas dan data-vip_farm**
                    const kapasitas = (opt.kapasitas_farm !== null && opt.kapasitas_farm !== undefined) ? opt.kapasitas_farm : '0';
                    
                    // --- MODIFIKASI LOGIKA VIP ---
                    // Cek jika opt.vip_farm ada, trim, dan samakan dengan 'Ya'
                    // Ini menangani "Ya", "Ya ", null, undefined
                    const vip_status = (opt.vip_farm && opt.vip_farm.trim() === 'Ya') ? 'Ya' : 'Tidak';
                    // --- AKHIR MODIFIKASI ---
                    
                    const escapedText = opt.option_text.replace(/'/g, "\\'");
                    
                    // **PENTING: kirim 'this' (elemen) ke selectOption dan tambahkan data-vip_farm**
...
                    formGroup.innerHTML += `
                        <div class="capacity-and-refresh-container">
                            <div id="kapasitas_caption_div" class="auto-calc-info farm-capacity-info">Kapasitas farm: -</div>
...
    }
    ...
    document.getElementById('petelurForm').addEventListener('submit', async function(e) {
        ...
        // 2. **VALIDASI KAPASITAS BARU (TERMASUK LOGIKA LAYER)**
...
            // Kita target error input ke field non-CP (yang terakhir) atau CP jika non-CP tidak ada
            errorInput = efektifNonCPInput || efektifCPInput; 
        }
