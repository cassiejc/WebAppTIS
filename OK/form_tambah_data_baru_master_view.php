...
<body>
                        ...
                        <?php elseif ($q['type'] == 'select'): ?>
                            ...
                            <?php elseif ($kategori_selected == 'Farm' && $q['field_name'] == 'tipe_ternak'): ?>
                                <select name="q<?= $q['questions_id'] ?>" 
                                        ...
                                        style="max-width: 400px"
                                        <?= !empty($q['required']) ? 'required' : '' ?>
                                        onchange="toggleVipFarm(this.value)"> <option value="">-- Pilih Jawaban --</option>
                                    <?php if (!empty($q['options'])): ?>
                                        <?php foreach ($q['options'] as $opt): ?>
    ...
    <script>
        ...
        function toggleVipFarm(selectedValue) {
            const vipFarmField = document.getElementById('field-vip_farm');
            if (!vipFarmField) {
                console.warn('Field "field-vip_farm" not found.');
                return;
            }

            if (selectedValue === 'Grower') {
                // Tampilkan field
                vipFarmField.style.display = 'block';
                
                // Cek apakah field ini 'required' dari data PHP
                const qData = window.questionsData.find(q => q.field_name === 'vip_farm');
                if (qData && qData.required == '1') {
                    // Tambahkan 'required' ke input di dalamnya
                    const inputs = vipFarmField.querySelectorAll('input, select, textarea');
                    inputs.forEach(input => {
                        input.setAttribute('required', 'required');
                    });
                }
                ...
                // Sembunyikan field
                vipFarmField.style.display = 'none';
                resetFieldInputs(vipFarmField); // Gunakan helper yang sudah ada
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (kategori === 'Farm') {
                const tipeTernakFarmSelect = document.getElementById('farm_tipe_ternak_select');
                if (tipeTernakFarmSelect) {
                    // Panggil fungsi saat load untuk set state awal
                    toggleVipFarm(tipeTernakFarmSelect.value); 
                }
            }
