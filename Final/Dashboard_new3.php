...
class Dashboard_new extends MY_Controller
{
    ...
    //     // 2. Tentukan tahun terpilih (default: tahun sekarang)
    ...
    //     // 4. Filter Tipe Ternak
    //     if ($this->input->method() == 'post') {
    //         $selected_tipe_ternak = $this->input->post('tipe_ternak');
    //     } else {
    //         $selected_tipe_ternak = ''; // Default tampilkan semua
    //     }
        
    //     // 5. Ambil data tabel kapasitas farm berdasarkan tahun
...
public function visual_kandang_kosong()
{
    ...
    if (!empty($selected_years)) {
        ...
        // Proses data menjadi format Chart.js
        $datasets = [];
        ...
        foreach ($raw_chart_data as $row) {
            $tahun = $row['tahun'];
            $bulan = (int)$row['bulan']; // 1-12
            $total_kosong = (int)$row['total_kosong'];
            
            if (!isset($data_by_year[$tahun])) {
                $data_by_year[$tahun] = array_fill(0, 12, 0); // Inisialisasi 12 bulan dengan 0
            }
            
            $data_by_year[$tahun][$bulan - 1] = $total_kosong; // Array index 0-11
        }
        ...
        $data['farm_capacity_list'] = [];
        $data['chart_data_monthly'] = json_encode([]); // Chart kosong
