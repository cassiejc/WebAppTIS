...
class M_master_harga extends CI_Model {
    ...
    /**
     * [COST KOMPLIT BROILER] Menghitung harga berdasarkan rumus komponen.
    ...
    public function proses_harga_komplit_broiler_harian($id_harga)
    {
        ...
        $harga_pre_stater = $this->_get_harga_by_nama('Harga Pakan Pre Stater');
        $harga_stater     = $this->_get_harga_by_nama('Harga Pakan Stater');
        $harga_finisher   = $this->_get_harga_by_nama('Harga Pakan Finisher');
        $ongkos_kirim     = $this->_get_harga_by_nama('Ongkos Kirim');

        if ($harga_pre_stater == 0 || $harga_stater == 0 || $harga_finisher == 0) {
            log_message('error', 'Gagal menghitung Cost Komplit Broiler: Salah satu harga pakan komponen bernilai 0.');
            return false;
        }

        $cost_pre_stater = ($harga_pre_stater + 150) * 0.0935; 
        $cost_stater     = ($harga_stater + 150) * 0.3115;     
        $cost_finisher   = ($harga_finisher + 150) * 0.5950;   

        $nilai_akhir = $cost_pre_stater + $cost_stater + $cost_finisher + $ongkos_kirim;
    ...
    //     $harga_doc_kemarin      = $this->_get_harga_bulanan_sebelumnya('harga_doc');
    //     $cost_komplit_broiler   = $this->_get_harga_terbaru_harian('cost_komplit_broiler');
    //     $konstanta_tambahan = (((650 * 100) / 0.96) + 600 + 3500) / 2;
    //     if ($harga_doc_kemarin == 0 || $cost_komplit_broiler == 0) {
    //         log_message('error', 'Gagal menghitung HPP Broiler: Harga DOC bulan kemarin atau Cost Komplit Broiler hari ini tidak ditemukan (bernilai 0).');
    //         return false;
    //     }

    //     $nilai_akhir = ($harga_doc_kemarin * 0.96) + ($cost_komplit_broiler * 3.21) + $konstanta_tambahan;
    ...
    public function proses_hpp_broiler_harian($id_harga)
    {
        ...
        // --- [PERUBAHAN DI SINI] ---
        // Memanggil fungsi baru T-1 Bulan (mundur)
        ...
        // --- [PERUBAHAN DI SINI] ---
        // Pengecekan diubah ke variabel baru
        ...
        $nilai_akhir = ($harga_doc_t_minus_1_month * 0.96) + ($cost_komplit_broiler * 3.21) + $konstanta_tambahan;
        // --- [AKHIR PERUBAHAN] ---
    ...
    // /**
    //  * [BARU] Fungsi helper PRIBADI untuk mengambil harga DOC 30 hari lalu.
    //  * Sesuai aturan HPP Broiler:
    //  * 1. Cek harga PADA atau SEBELUM T-30 hari (mencari mundur).
    //  * 2. Ambil 1 data terbaru dari pencarian mundur tsb.
    //  *
    //  * @return float Nilai harga, atau 0 jika tidak ditemukan.
    //  */
    // private function _get_harga_doc_for_hpp()
    // {
    //     $jenis_harga = 'harga_doc';
    //     $target_date = date('Y-m-d', strtotime('-30 days'));

    //     // 1. Cari harga PADA ATAU SEBELUM T-30 (mundur)
    //     $this->db->select('nilai_rata_rata');
    //     $this->db->from('harga_rata_rata_harian');
    //     $this->db->where('jenis_harga', $jenis_harga);
    //     $this->db->where('tanggal <=', $target_date); // Pada T-30 atau T-31, T-32...
    //     $this->db->order_by('tanggal', 'DESC');     // Ambil yang paling baru (T-30, atau T-31)
    //     $this->db->limit(1);
    ...
    //     // 2. Fallback: Jika tidak ada data MUNDUR sama sekali (misal: data baru ada T-29),
    //     //    kita tetap harus cari MAJU agar HPP tidak 0.
    //     $this->db->select('nilai_rata_rata');
    //     $this->db->from('harga_rata_rata_harian');
    //     $this->db->where('jenis_harga', $jenis_harga);
    //     $this->db->where('tanggal >', $target_date); // Tanggal SETELAH T-30 (T-29, T-28...)
    //     $this->db->order_by('tanggal', 'ASC');      // Ambil yang paling dekat (T-29)
    //     $this->db->limit(1);
    ...
    public function recalculate_all_prices()
        {
            ...
            // --- 11. Proses Harga Konsentrat Layer (Turunan) ---
            ...
            // --- 12. Proses Cost Komplit Broiler (Turunan) ---
            $id_cost_broiler = $this->_get_id_by_nama_harga('Average Cost Komplit Broiler');
            if ($id_cost_broiler) {
                // Bergantung pada: Manual 'Harga Pakan Pre Stater', 'Stater', 'Finisher', 'Ongkos Kirim'
                $this->proses_harga_komplit_broiler_harian($id_cost_broiler);
