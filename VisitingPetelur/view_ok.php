...
<body>
    <div class="container-fluid">
        <h2 class="page-title mb-4">Petelur</h2>
    ...
    function shouldBeIntegerType(f) { return ['efektif_terisi_petelur', 'doa_woa_petelur', 'layer_pakai_pakan_cp', 'layer_selain_pakan_cp', 'layer_jumlah_kandang', 'layer_lama_puncak_produksi', 'layer_populasi', 'layer_woa', 'layer_umur_tertua', 'layer_umur_termuda'].includes(f); }
    ...
    function createQuestionElement(q) {
        ...
        if (q.type === 'select' && q.options) {
            ...
            formGroup.innerHTML += `<textarea ${commonAttrs} rows="3" placeholder="Masukkan catatan..." style="resize: none; overflow: hidden; scrollbar-width: none; -ms-overflow-style: none;" oninput="autoResize(this)"></textarea>`;
        } else { // Handle 'text' dan 'number' dengan validasi spesifik
            let input;
            if (shouldBeCurrencyType(q.field_name)) {
                input = `<div class="currency-input"><span class="currency-prefix">Rp</span><input type="text" ${commonAttrs} placeholder="Masukkan angka" oninput="formatWithComma(this)" onkeypress="return isIntegerKey(event)"></div>`;
                ...
                input = `<input type="text" inputmode="decimal" ${commonAttrs} placeholder="Masukkan angka desimal" onkeypress="return isNumberKey(event)">`;
            } else {
