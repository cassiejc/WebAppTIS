...
<body>
                        ...
                        <?php if ($q['type'] === 'select' && !empty($q['options'])): ?>
                            <?php if ($q['field_name'] === 'nama_farm'): ?>
                                <div class="dropdown">
                                    <button onclick="toggleDropdown('dropdown_<?= $q['questions_id'] ?>')" 
                                            class="dropbtn <?php if ($q['field_name'] === 'nama_farm' || strpos($q['field_name'], 'strain') !== false || strpos($q['field_name'], 'pakan') !== false): ?>disabled<?php endif; ?>" 
                                            type="button" 
                                            id="dropbtn_<?= $q['questions_id'] ?>"
                                            <?php if ($q['field_name'] === 'nama_farm' || strpos($q['field_name'], 'strain') !== false || strpos($q['field_name'], 'pakan') !== false): ?>disabled<?php endif; ?>>-- Pilih Nama Farm --</button>
                            ...
                            <?php else: ?>
                                <select name="q<?= $q['questions_id'] ?>" class="form-control" 
                                        ...
                                        <?php if (strpos($q['field_name'], 'strain') !== false || strpos($q['field_name'], 'pakan') !== false): ?>disabled<?php endif; ?>
    ...
    <script>
        ...
        function enableDependentDropdowns(enable) {
            ...
            // Handle strain and pakan selects
            const allSelects = document.querySelectorAll('select[class*="form-control"]');
            allSelects.forEach(select => {
                const questionGroup = select.closest('.form-group');
                const label = questionGroup.querySelector('label');
                
                if (label && (label.textContent.toLowerCase().includes('strain') || 
                             label.textContent.toLowerCase().includes('pakan') ||
                             select.name.includes('strain') || 
                             select.name.includes('pakan'))) {
                    if (enable) {
                ...
                questions.forEach(function(q) {
                    ...
                    if (q.type === 'select' && q.options && q.options.length > 0) {
                        if (q.field_name === 'nama_farm' || q.field_name === 'layer_nama_farm') {
                            ...
                            // Disable fields that contain 'strain' or 'pakan' in field_name
                            if (q.field_name.toLowerCase().includes('strain') || 
                                q.field_name.toLowerCase().includes('pakan')) {
                                select.disabled = true;
                            }
